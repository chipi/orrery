...
**Moon regional context layer = Kaguya TC, not LROC mosaic (#361).** The Moon has no global CTX-style mosaic wired. The 12 detail-only /moon landers (Luna 9/16/17/21/24, Chang'e 3/4/5/6, Chandrayaan-3, SLIM, Beresheet) get their CTX-equivalent regional patch from JAXA's **SELENE (Kaguya) Terrain Camera** via `scripts/hotspots/fetch-moon-kaguya-regional.ts`: per site it STAC-searches the USGS Astrogeology ARD collections (`kaguya_terrain_camera_{monoscopic,stereoscopic,spsupport}_uncontrolled_observations`) by bbox, ranks candidates (footprint-containing first, then nearest), and `gdal /vsicurl/` **window-crops** a 2560² (~16 km) patch from the public S3 COG (`image` asset, ~6–12 m/px) — no full-file downloads. Writes `static/images/hotspots/moon/<site>/tier2-regional.jpg` + self-credits `buildKaguyaTcProvenanceEntry` (agency JAXA, `JAXA-OPEN`, `instrument:'Kaguya TC'`, no `spacecraft_id` yet → no dead 📡 chip). `/credits` routes `instrument==='Kaguya TC'→'jaxa-kaguya-tc'` (its own **JAXA · Kaguya TC** section). Run: `~/.nvm/.../v20.20.2/bin/node --import tsx scripts/hotspots/fetch-moon-kaguya-regional.ts [siteId]`. Adding the regional source flips `haloHasRegional` so the selection bracket sizes to the 3.0u regional patch (see SurfaceScene halo-radius). NOTE: `image-provenance.json` is a concurrent-write hotspot — `build-image-provenance.ts` preserves existing entries (`buildWalkerFallbackEntries(existing)` skips covered paths), but a parallel run can clobber freshly-upserted entries via a load-snapshot race; re-run the fetcher + verify before committing.

**Surface select-landing zoom + Tier reveal (#361).** Selecting a surface site flies to `faceCameraAtSite` default `targetR = 37` (was 50 — that landed before the regional ramp even started, so you saw a bare globe). Regional ramp `regionalFadeStart 44 → 33` (was 50→33), detail `33 → 30.32`, flat-patch `30.1`. At camR 37 the regional context patch is ~64% revealed and bracket-framed on arrival; lean in to sharpen to HiRISE/LROC detail. Both the label-fade `regionalFadeStartTop` and the patch `regionalFadeStart` must stay in lock-step.
