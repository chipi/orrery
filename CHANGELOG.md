...
- **`/fly` catalog backfill — 14 notable missions across 5 batches** (GH #341). Closes the post-throne-of-glory gap surfaced in the Batch 5 sweep — every mission now ships with mission JSON + flight.events + interplanetary_profile + i18n overlay + fleet refs + index entry.
  - **Batch 1 — Mars rover quad**: Mariner 9 (1971 — first orbiter of another planet), Spirit + Opportunity (MER twins, 2003), Phoenix (2007 — first confirmation of water ice in Martian arctic regolith). Commit `064d37bf8`.
  - **Batch 2 — Venus pair**: Magellan (1989 — radar-mapped 98% of Venus + first planned planetary atmospheric entry), Akatsuki (JAXA, 2010 — recovered from 2010 VOI failure on a 2015 backup attempt using attitude-control thrusters). Commit `34e02726e`.
  - **Batch 3 — Sample-return pair**: OSIRIS-REx (2016 — 121.6 g Bennu sample returned 2023; now OSIRIS-APEX en route to Apophis 2029), Psyche (2023 — first mission to a metal-rich M-class asteroid, ion propulsion + Mars assist). Commit `3516d05ea`.
  - **Batch 4 — Sun-skirter pair**: Parker Solar Probe (NASA 2018 — closest spacecraft to Sun ever flown, 9.86 R☉; first to enter the corona), Solar Orbiter (ESA / NASA 2020 — first detailed Sun-polar imaging via 33° heliographic inclination). Commit `72aedc105`.
  - **Batch 5 — New-body sprawl**: DART (2021 — first asteroid deflection by humans), Lucy (2021 — first mission to Jupiter Trojans, 12-year multi-flyby tour), Europa Clipper (2024 — largest planetary mission spacecraft, 49 Europa flybys), Hayabusa 1 (JAXA, 2003 — first asteroid sample-return). Commit `48c42ed5e`.
  - **Body-wiring sprint** (commit `91b8ed0db`): 10 new destination bodies first-class on `/fly` — Itokawa, Didymos+Dimorphos, Donaldjohanson, Eurybates+Polymele+Leucus+Orus+Patroclus+Menoetius — wired across all 15 §body-wiring touchpoints (DestinationId / PLANET_SIZES / PLANET_COMPOSITION / DEST_STYLE / FLYBY_RADIUS / labelToPlanetId / NON_CONTEXT_BODIES / DESTINATION_LABEL_COLORS / /plan switch / 14-locale messages / `/explore` planet overlays). DART now composes Dimorphos at impact; Lucy composes each Trojan at flyby; Hayabusa 1 composes Itokawa at station-keeping. The hero shot lands.
  - **Schema extensions**: `mission.schema.json` + `mission-index.schema.json` `dest` + `arrival_body` enums extended for small-body destinations; `validate-data` waypoint-radius lower bound dropped 0.1 → 0.04 AU for Sun-grazers; `validate-hero-coverage` FALLBACK_LADDER extended so missions can resolve via mars-sites / moon-sites / fleet-galleries gallery.
