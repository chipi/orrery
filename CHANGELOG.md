...
- **/fly red-dot vs blue-tube split — final fix** (issue #228). v0.6.1 snapped the past-tube tip to the sprite; v0.6.3 finds the residual split this didn't catch. The dim "future" tube (full-arc preview, low opacity, drawn over the past tube) kept its csIdx vertex at the original `pts[csIdx]` position — up to one segment AHEAD of the sprite for any fraction not on a vertex boundary. So the bright tube ended at the sprite, the dim tube continued from a position ahead of it, and the user saw a ~0.5-1 scene-unit gap. Fix: `snapTubeTip` now takes a `role: 'past' | 'future'` arg. Both meshes have their csIdx cross-section translated to the sprite (no split possible — the bright/dim transition shares a vertex). Past-tube drawRange stops at csIdx; future-tube drawRange covers the full arc.
