...
> 3. **The coverage gate.** CI enforces functions ≥ 82% via `test:coverage`; preflight skips it. New lib/ code with untested functions (e.g. data.ts loaders) silently drops the ratio — run `npm run test:coverage` and recover any regression by ADDING tests, never by lowering the threshold. **Always watch the CI run to green after pushing to `main`** — don't assume preflight-green == CI-green. (The 2026-07-16 /explore v2 → main land ate three red pushes to this exact trio.)
