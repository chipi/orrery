...
4. **The `README.md` of every folder you're about to edit** (module map,
   where one exists) — which file owns what, which look-alike to touch, and
   the local conventions. See AGENTS.md §Module READMEs.
...
- **`Agent` (subagent) with `Explore`** for codebase research that would take 4+ Bash/Grep calls. Particularly useful for cross-route audits like "what's the actual route inventory" or "where do we use feature X".
  - **Subagents delegate DOWN — never inherit Opus.** Set an explicit `model` on every `Agent` call: `haiku` for mechanical/bounded work (implementers applying a spec, test writers, doc edits, file transforms), `sonnet` for judgment work (reviewers, fact-checkers, planners). `opus` is escalation-only, when the user says a task needs it — never the default. Project-defined agents (`.claude/agents/*.md`) already pin this in frontmatter (e.g. `science-reviewer: model: sonnet`); the same tier applies to the built-in `general-purpose` / `implementer` / `Explore` types via the per-call `model` param. For large sweeps, batch harder (15–20 items/agent) to cut agent count, and surface the token/cost curve **before** fanning out dozens of agents, not after.
...
> **Preflight is NOT a byte-for-byte CI mirror — three gaps to close after a rebase/merge.** CI runs a cold `npm ci` (no lint cache) + `test:coverage`; preflight runs against your *installed* node_modules with `prettier --check --cache` / `eslint --cache` and only `npm run test` (no coverage). So a locally-green preflight can go red on CI three ways:
> 1. **Stale tooling after a rebase.** A rebase/merge that changes `package-lock.json` (e.g. eslint 9→10, a prettier-plugin bump) leaves node_modules on the OLD versions — local lint passes with looser tooling, CI's fresh `npm ci` fails. **After any rebase/merge that touches the lockfile, run `npm ci` before trusting preflight** (`npx eslint --version` should match the lock).
> 2. **Lint caches mask cold issues.** Reproduce CI with `npx prettier --check --no-cache .` and `npx eslint --no-cache .`.
> 3. **The coverage gate.** CI enforces coverage floors (functions/statements/branches/lines — see `vite.config.ts` thresholds) via `test:coverage`; preflight skips it. New lib/ code with untested functions (e.g. data.ts loaders) silently drops the ratio. Before pushing lib changes, run **`npm run preflight:coverage`** (preflight + the full coverage gate in one) and recover any regression by ADDING tests, never by lowering the threshold — the thresholds are a frozen ratchet (R8). **Always watch the CI run to green after pushing to `main`** — don't assume preflight-green == CI-green. (The 2026-07-16 /explore v2 → main land ate three red pushes to this exact trio.)
