...
**No IndexedDB, no third-party fonts at runtime** (every asset resolved at build time per [ADR-016](docs/adr/ADR-016.md)). Client-side state is deliberately minimal and carries no PII: **two functional cookies** — `orrery_locale` (1-year, written *only* when you click a locale chip; auto-detected locales never write it — [ADR-057](docs/adr/ADR-057.md)) and `orrery_tour` (30-day, Curator-Tour resume position — [ADR-075](docs/adr/ADR-075.md)), both `SameSite=Lax` — plus **one `localStorage` key** (`orrery.qualityTier`, your graphics quality-tier preference; a performance setting, no PII). Locale preference also lives in the URL — bookmark `?lang=ja` and you have your locale; share it and they have theirs.
