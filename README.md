...
**No backend. No database. No personal data.** A static SPA you can self-host, install as a PWA, and use offline after first load. The only telemetry on the live site (`orrerylearn.com`) is **self-hosted**, aggregate-only usage analytics via [Umami](#privacy) plus crash reports — cookieless, no fingerprinting, no PII, no cross-site tracking, and never used to identify you.
...
Available in **14 languages, all at 100% UI parity**: English · Español · Français · Deutsch · Português (BR) · Italiano · Nederlands · 中文 · 日本語 · 한국어 · हिन्दी · العربية (RTL) · Русский · Српски (Cyrillic). Browser locale is auto-detected on first visit (per ADR-057, locale picks persist via a small functional cookie — no tracking; see [Privacy](#privacy) for the full, non-PII client-state list).
...
Plus four read-only pages: **`/credits`** (per-image provenance manifest + text-source attributions), **`/library`** (bill-of-links across the entire app — every outbound LEARN link with native-language priority and freshness gating), **`/posters`** (27 original art-print posters in three style families — vintage screen-print, modern comic, photoreal & future — AI-generated, then captioned and ORRERY-wordmarked in-house; click any poster for the full-resolution JPG), and **`/colophon`** (the bill-of-materials for Orrery's *original* content — every hand-made poster, diagram, anatomy plate, and 3D model, with how each was produced).
...
- **Take a tour** — two auto-advancing playlists: the **Curator Tour** (21 episodes, ~66 min) and the longer **Curator Extended Tour** (31 episodes, ~87 min, interleaving 10 enthusiast deep-dives). Route-by-route auto-navigation, prev / stop / next controls, timed directive banners, and resume-where-you-left-off (the `orrery_tour` cookie, ADR-075). The complete episode inventory is on [`/colophon`](https://chipi.github.io/orrery/colophon).
...
**Analytics — [Umami](https://umami.is/), self-hosted, aggregate-only.** The live site at `orrerylearn.com` loads a single Umami `script.js` from our **self-hosted** Umami at `analytics.orrerylearn.com` (behind Cloudflare, origin-locked — the data never leaves our own infrastructure to a third party). It records anonymous **page-view counts per route** and a handful of structured custom events (`mission-load`, `lens-toggle`, `science-section-view`, `locale-switch` — see [`src/lib/analytics.ts`](src/lib/analytics.ts) for the full list). That is the entire collected dataset, and it **never identifies individuals** — it is aggregate usage only.

Umami is **cookieless**, does **no fingerprinting**, captures **no PII**, has **no cross-site tracking**, and is GDPR / CCPA-compliant out of the box. Telemetry runs on a **three-rung environment ladder** so the public prod dataset only ever reflects real visitors: **prod** (`orrerylearn.com`) reports to its own Umami site; **staging** (the `chipi.github.io/orrery` gh-pages deploy) reports to a *separate* Umami site tagged `environment=staging`; and the maintainer's **local `vite dev`** reports to a *third*, dedicated dev site reachable only over our private Tailscale network. `vite preview`, the screenshot pipeline, and e2e runs stay silent (no baked config). Self-hosters who clone Orrery get **zero analytics by default** — the prod/staging env vars aren't in their CI, and the dev site's host resolves only on our tailnet, so a fork is silent *by construction* until they point it at their own Umami ([ADR-081](docs/adr/ADR-081.md)).

**No IndexedDB, no third-party fonts at runtime** (every asset resolved at build time per [ADR-016](docs/adr/ADR-016.md)). Client-side state is deliberately minimal and carries no PII: **three functional cookies** — `orrery_locale` (1-year, written *only* when you click a locale chip; auto-detected locales never write it — [ADR-057](docs/adr/ADR-057.md)), `orrery_tour` (30-day, Curator-Tour resume position — [ADR-075](docs/adr/ADR-075.md)), and `orrery_analytics_optout` (1-year, written only if you switch analytics off on [/privacy](https://www.orrerylearn.com/privacy) — [ADR-092](docs/adr/ADR-092.md)), each written only on a deliberate action (`orrery_tour` and `orrery_analytics_optout` are `SameSite=Lax`; `orrery_locale` is written by Paraglide, which sets only `path` + `max-age` — see the [ADR-057](docs/adr/ADR-057.md) amendment) — plus a few device-local `localStorage` keys for rendering preferences (`orrery.qualityTier`, debug mode, offline downloads); settings, no PII. Locale preference also lives in the URL — bookmark `?lang=ja` and you have your locale; share it and they have theirs.

**Error tracking — [Sentry SDK](https://sentry.io/) → self-hosted [GlitchTip](https://glitchtip.com/), errors only, no PII, fork-silent.** The SDK sends only unhandled JavaScript exceptions and unhandled promise rejections to our **self-hosted GlitchTip** at `telemetry.orrerylearn.com` — not a third-party SaaS — on the same three-rung ladder as analytics: **prod**, **staging**, and the maintainer's **local `vite dev`** each report to a *separate* GlitchTip project (tagged `environment=prod|staging|dev`, and in dev also by git worktree), so an error's origin is never ambiguous and dev noise never lands in the prod project. No performance tracing, no Web Vitals, no session replay. `sendDefaultPii: false` is set explicitly, and a `beforeSend` hook strips URL query parameters and hash, nulls request headers + cookies, and sets `user.ip_address` to `0.0.0.0` (the discard sentinel); `ui.input` breadcrumbs are dropped entirely. Self-hosters who clone Orrery and build their own fork get **zero error traffic by default** — the prod/staging DSN isn't in their CI secrets, and the dev DSN's host resolves only on our tailnet, so the SDK stays silent *by construction* before any network request fires ([ADR-067](docs/adr/ADR-067.md)). See [`src/lib/observability/sentry.ts`](src/lib/observability/sentry.ts) for the full scrubber.
...
**If you want to add a mission:** read [03 Data Catalog](docs/concept/03_Data_Catalog.md) for the JSON schema; then [Mission trajectories](docs/guides/mission-trajectories.md) for the waypoint + phase-marker workflow.
