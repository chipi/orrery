...
Umami is **cookieless**, does **no fingerprinting**, captures **no PII**, has **no cross-site tracking**, and is GDPR / CCPA-compliant out of the box. The tracker loads only when the production build bakes in the `PUBLIC_UMAMI_*` config — local dev, `vite preview`, screenshot pipeline, e2e runs are all silent so the dataset reflects real visitors, not engineer reloads. Self-hosters who clone Orrery get **zero analytics by default** — the env vars aren't in their CI, so a fork is silent until they point it at their own Umami ([ADR-081](docs/adr/ADR-081.md)).
...
**Error tracking — [Sentry SDK](https://sentry.io/) → self-hosted [GlitchTip](https://glitchtip.com/), errors only, no PII, fork-silent.** Production builds initialise the Sentry SDK when `PUBLIC_SENTRY_DSN` is set at build time, sending only unhandled JavaScript exceptions and unhandled promise rejections to our **self-hosted GlitchTip** at `telemetry.orrerylearn.com` — not a third-party SaaS. No performance tracing, no Web Vitals, no session replay. `sendDefaultPii: false` is set explicitly, and a `beforeSend` hook strips URL query parameters and hash, nulls request headers + cookies, and sets `user.ip_address` to `0.0.0.0` (the discard sentinel); `ui.input` breadcrumbs are dropped entirely. Self-hosters who clone Orrery and build their own fork get **zero error traffic by default** — the env var isn't in their CI secrets, so the SDK init returns early before any network request fires ([ADR-067](docs/adr/ADR-067.md)). See [`src/lib/observability/sentry.ts`](src/lib/observability/sentry.ts) for the full scrubber.
