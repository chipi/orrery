...
- **Google Fonts** — downloaded via `scripts/fetch-assets.ts`, committed to `static/fonts/`
- **Planet textures** — downloaded from Three.js r128 repo during build, written to `static/textures/`
- **Agency logos** — downloaded from Wikimedia Commons during build, written to `static/logos/`
- **Mission imagery** — NASA Images API queried per mission during build, top result per mission written to `static/images/missions/[id]/`. Scheduled rebuild (weekly) keeps active mission imagery fresh.
