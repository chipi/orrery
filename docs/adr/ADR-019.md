...
`data/schemas/mission.schema.json` defines required fields and types. `scripts/validate-data.js` runs `ajv` against all files in `data/missions/`. CI step: `node scripts/validate-data.js`. Runs on PR and on push to main.
