...
> Scope · Wave 1 only; CJK + RTL deferred
...
**Wave 2 (CJK — `zh-CN`, `ja`, `ko`): deferred.**

A follow-up ADR (planned id ADR-0xx) will decide between:
- Per-locale Noto Sans (SC / JP / KR) loaded on demand
- Single Noto Sans CJK variable font for all three locales
- System-font fallback stack with controlled tofu acceptance

The decision is gated on a font-loading-strategy spike when Wave 2 is scheduled.
...
- Arabic RTL: choose between CSS logical properties throughout (`margin-inline`, `padding-block`) vs an `rtl.css` override stylesheet. This is the largest infra delta in the i18n programme and warrants its own ADR.
...
- Wave 2 / Wave 3 follow-up ADRs should reference this ADR's "deferred" decision and document what triggered them being scheduled (typically: a per-language ticket entering active development).
