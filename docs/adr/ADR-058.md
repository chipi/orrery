...
> Status · Accepted
> Date · 2026-05-13 (original); 2026-05-28 (third amendment — full corpus rollout)
...
## Amendment — 2026-05-28: Hybrid Tier 2 + per-phase science chips (#107 Apollo 11 first slice)

**The original ADR sketched Tier 2 as "published waypoints from NASA reconstructions" with the implicit expectation of ~50–100 NASA-tabulated ECI state vectors per mission. That public data does not exist in directly transcribable form** — NASA Mission Reports (MSC-00171 for Apollo 11, MSC-02680 for Apollo 13, JSC-07904 for Apollo 17) carry event-anchored state vectors at TLI cutoff / LOI / TEI / reentry but not continuous 100-point tables; JPL Horizons + SPICE do not cover Apollo records. The same applies to Artemis II until post-flight reconstruction publishes.

Tier 2 is therefore **redefined as "hybrid Tier 2"**:

- **Anchors** (typically ~10 per mission) are the NASA-published parametric values already in each mission JSON's `cislunar_profile` parametric fields (parking_orbit.altitude_km, tli.dv_kms, lunar_arrival.periselene_km, lunar_arrival.inclination_deg, return.dv_kms). These match the published reality recorded in MSC-00171 / equivalent.
- **Interpolated samples** between anchors are produced by the same cislunar-geometry primitives that drive Tier 1 (Keplerian arcs anchored at the parametric endpoints) and are physics-correct by construction.
- **Materialisation** is done once by `scripts/cislunar/generate-hybrid-waypoints.ts` (renamed from `generate-tier2-waypoints.ts` after the v0.7 wrap-up tier-honesty pass), which calls `buildCislunarTrajectory()` on the Tier 1 profile, resamples to 100 MET-evenly-spaced waypoints, pins event MET anchors to exact slot METs, and writes back to `waypoints_km` + flips `source_tier` to `tier_1_5_hybrid` (the resampled curve is geometrically faithful but not real published state vectors — `tier_2_published` is reserved for direct NASA TND ingest).

**Value over Tier 1:** the trajectory becomes a frozen, reviewable dataset with deterministic phase-marker positions (waypoint index N corresponds to a specific event's MET). A future change to a Tier 1 generator no longer silently shifts Apollo 11's rendered shape; re-running the generator becomes an explicit, reviewable diff. The renderer also skips per-frame trajectory recomputation.

### Per-phase science chips (GH #107 commit 5–8)

The cislunar phase + flight-event types now link to `/science` encyclopedia sections via `static/data/cislunar-phase-science-map.json` and a pure helper module `src/lib/cislunar-events.ts`. The map keys on `CislunarPhaseType` (phase_refs) and `FlightEventType` (event_refs); each entry is an ordered array of `{tab, slug}` — first entry is the primary chip.

Surfaces consuming the map:
- **HUD phase pill** — derived ScienceChip next to `phaseLabel`. Moon missions use `currentPhaseFor(simMet, trajectory)` to pick the cislunar-phase-specific section; other missions use a coarse heliocentric fallback (transfer-ellipse, edl).
- **Trajectory phase markers** — one `<PhaseMarkerLabel>` per `flight.events[]` entry, positioned via the pure projection module `src/lib/cislunar-screen-projection.ts` (3D + 2D dual coverage), reveal-animated via `src/lib/cislunar-marker-reveal.ts`. Each marker carries the per-event `primaryScienceRefFor({ eventType })` chip.

A preflight gate (`scripts/cislunar/check-science-map-refs.ts`, chained into `validate-data`) asserts every (tab, slug) in the map resolves to an existing `/science/<tab>/<slug>.json` file — a typo in the map fails at commit time rather than producing a 404 chip in the UI.

### Apollo 11 as the scaling template

Apollo 11 is the **gold-standard reference** for this work: 100 waypoints, 8 event anchors, full 3D + 2D rendering, science-chip cross-links, 14-locale event labels. The follow-up rollout (Apollo 13/17 + Artemis II) reuses the same generator script + science map + UI surface — no per-mission code changes. Each follow-up mission is a single `npx tsx scripts/cislunar/generate-hybrid-waypoints.ts` invocation against its JSON.

## Amendment — 2026-05-28 (v0.7.x #107 Step 6): Full corpus rollout — 42 missions

The Apollo 11 architecture now ships across **every mission in the corpus** (Moon + Mars + outer-system, 42 missions total) via the following extensions:

**Schema** — NEW `flight.interplanetary_profile` field parallels `cislunar_profile` for heliocentric trajectories. Carries `source_tier` (same 3-tier enum), `reference_frame` (`heliocentric_ecliptic_J2000`), `departure_body` (`earth`), `arrival_body` (mars/ceres/jupiter/neptune/pluto), `transfer_type` (hohmann/type1_lambert/type2_lambert), `tcm_count`, and `waypoints_helio_au[]` (Tier 1.5/2 only — `[met_days, x_au, y_au, z_au]` tuples).

**Geometry** — NEW `src/lib/interplanetary-geometry.ts` mirrors `cislunar-geometry.ts`. `buildInterplanetaryTrajectory(profile, opts)` dispatches on `waypoints_helio_au` presence → `buildFromHelioWaypoints` (Tier 1.5/2 replay); otherwise falls back to a parametric path via `transferEllipse()` from `mission-arc.ts`. Eleven new phase types cover the interplanetary timeline (`parking_earth, tmi_coast, helio_cruise, mid_course, arrival_approach, arrival_orbit, mars_descent, mars_surface, mars_ascent, tei_helio, earth_return_helio`).

**Pure helpers** — NEW `src/lib/interplanetary-events.ts` (`currentInterplanetaryPhaseFor`, `phaseMarkerAuPositions`, `primaryInterplanetaryPhaseScienceRef`), NEW heliocentric projection helpers `helioAuToScreenPx` + `helioAuToCanvas2dPx` in `cislunar-screen-projection.ts`.

**Generators** — `scripts/cislunar/generate-hybrid-waypoints.ts` (Moon) ran on all 20 remaining Moon missions in 6 batched commits. NEW `scripts/cislunar/generate-helio-hybrid-waypoints.ts` (Mars + outer-system) ran on all 14 Mars + 4 outer-system missions. All ship at `tier_1_5_hybrid` (synthetic geometry from analytic primitives + event-MET pinning) — `tier_2_published` is now formally reserved for direct NASA TND / ESA SPICE state-vector ingest (deferred to v0.8+).

**Gating** — `/fly`'s `isMoonMission` reference frame gate stays for cislunar-vs-heliocentric scene selection. NEW `hasPhaseMarkers` derived flag (true for Moon OR Mars with events + trajectory) gates the phase-marker compute + render + overlay sites. Both 3D animate loop and 2D `draw2d` dispatch projection per-mission (ECI km via `eciKmToScreenPx`/`eciKmToCanvas2dPx` for Moon, helio AU via `helioAuToScreenPx`/`helioAuToCanvas2dPx` for Mars+).

**Science map** — `cislunar-phase-science-map.json` extended from 10 cislunar phases to 21 (cislunar + heliocentric); 14 event types covered (added phasing → trans-x-injection + dv-budget, separation → edl, anomaly → free-return). All slugs resolve via the existing `check-science-map-refs.ts` preflight gate.

**i18n** — 2 missing event labels added (`fly_event_phasing`, `fly_event_separation`); all 15 fly_event keys present in all 14 locales (uppercase aerospace shorthand convention — EN-baseline across non-EN locales, same as TCM/EDL/ANOMALY).

**Scrubber UX** — phase marker dots are now `<button>` elements with `onJump(metDays)` callback. Clicking jumps `simDay` to `depDay + met`, pauses sim. Keyboard-focusable with aria-label conveying the navigation effect. Reduced-motion: snap, no animation.

**Tests** — `tests/e2e/fly-apollo11-phase-markers.spec.ts` (15 tests × 2 projects = 30) is the depth reference. NEW `tests/e2e/fly-phase-markers-parity.spec.ts` covers all 39 events-populated missions with 5 lightweight smoke assertions each (~390 assertions per project). NEW `src/lib/mission-waypoint-snapshots.test.ts` golden-snapshots waypoint counts + endpoint METs for every mission (39 vitest tests). Tier 1 → Tier 1.5 reclassification protected by typecheck (`CislunarSourceTier` + `InterplanetarySourceTier` enums).

After this amendment, GH #107's full scope (Apollo 11 → all missions) is closed. The reserved `tier_2_published` path remains open as a follow-up issue for v0.8+ when NASA Mission Report PDFs are ingested as canonical state vectors.
