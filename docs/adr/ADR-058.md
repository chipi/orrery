...
- **Materialisation** is done once by `scripts/cislunar/generate-tier2-waypoints.ts`, which calls `buildCislunarTrajectory()` on the Tier 1 profile, resamples to 100 MET-evenly-spaced waypoints, pins event MET anchors to exact slot METs, and writes back to `waypoints_km` + flips `source_tier` to `tier_2_published`.
