...
**600 ms ease-in-out cubic**. Same duration as Slice 2's fly-in tween (Drift 14) for consistency. During the transition:

- Sphere scene opacity ramps 1 → 0 over the first 60 % of the window.
- Flat patch opacity ramps 0 → 1 over the last 80 %, overlapping the sphere fade-out for 40 % of the window.
- Three.js `renderer.domElement` continues animating (auto-spin paused) so the user perceives the sphere "falling away" rather than freezing.
- After the transition completes, the Three.js renderer is paused (no RAF) until the back gesture; flat patch owns the frame budget.
