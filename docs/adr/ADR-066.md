...
**Draft — to be Accepted with the docker-stack Slice 2 PR (the one that adds this workflow).** Implementation is one new YAML file + a one-line `playwright.config.ts` change. The future VPS RFC will extend this workflow to assert against a prod overlay; the base contract here doesn't change.
