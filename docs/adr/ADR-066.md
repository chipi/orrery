...
> Status · Accepted (Trigger surface, "desktop-chromium only", Relationship to existing workflows, and Branch protection sections superseded by ADR-071)
> Date · 2026-05-22
> Closes into · RFC-024 §7
> Related ADRs · ADR-015 (Playwright e2e + adapter-static), ADR-063 (web container), ADR-064 (pipeline runner), ADR-065 (live data volume), **ADR-071 (consolidation of e2e + docker-e2e — docker-e2e becomes the canonical Playwright gate; e2e.yml deleted)**
