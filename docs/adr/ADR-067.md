...
- `PUBLIC_SENTRY_DSN= npm run build` succeeds, fork-silent path (SDK no-op) confirmed by absence of DSN in bundle.
- `PUBLIC_SENTRY_DSN=<test-DSN> npm run build` succeeds, DSN inlined into `build/_app/env.js`, SDK code bundled into `build/_app/immutable/entry/app.*.js` (158 KB chunk).
- `npm run typecheck`: 0 errors / 0 warnings. `npm run lint`: clean. `npm run test`: 1141/1141 passing. `npm run check-no-secrets:all`: 8801 files clean.
...
env:
  ...
  PUBLIC_SENTRY_ENVIRONMENT: production
  PUBLIC_SENTRY_RELEASE:     gha-${{ steps.sha.outputs.short }}
```

`PUBLIC_SENTRY_DSN` is staged in the GH `prod` environment (not at the repo level — keeps it gated by the environment's protection rules). Empty/unset still no-ops per the original ADR-067 fork-silent design. The `gh-pages` preview deploy still reads from the never-set `SENTRY_DSN_WEB` repo secret — that's an inconsistency worth squashing in a follow-up, but not in #263's scope.

### Amendment — 2026-07-24 (ADR-082, telemetry env ladder) — reads that are now stale

The binary "deploy is live / everything else silent" posture in this ADR was
extended into a three-rung ladder by **[ADR-082](ADR-082.md)** — read it as the
current source of truth. What changed:

- **The self-hosted backend is GlitchTip, live at `telemetry.orrerylearn.com`.**
  Any "self-hosted GlitchTip — *Rejected*" line in this ADR's Alternatives is
  **historical only** — that decision was reversed; GlitchTip is the actual error
  sink (Sentry SDK → self-hosted GlitchTip, never Sentry SaaS).
- **`vite dev` is no longer hard-silent.** The `if (dev) return` guard is gone;
  dev now falls back to a dedicated dev DSN (GlitchTip **project 7**) reachable
  only over the Tailscale `homelab` host. Fork-silence is preserved *by
  construction* (that host is unreachable off the tailnet), not by the guard.
- **`preview.yml` → `staging.yml`**, and the `SENTRY_DSN_WEB` inconsistency
  flagged above is resolved: the gh-pages deploy now reads `PUBLIC_SENTRY_DSN`
  from a dedicated `staging` GH environment, tagged `environment=staging`
  (GlitchTip **project 6**), so it never pollutes prod (**project 4**).
- Per-env projects: **prod = 4, staging = 6, dev = 7**; dev events also carry a
  `worktree` tag (git branch) so parallel local sessions stay distinguishable.


## Amendment — 2026-09-10 (crash reports follow the analytics opt-out)

`src/hooks.client.ts` now calls `initSentry()` only when `analyticsSuppressed()`
is false — the ADR-092 opt-out cookie, `navigator.globalPrivacyControl`, or
`navigator.doNotTrack`. `/privacy` tells the user the toggle stops the crash
reports it describes, and that has to be true.

**The cost, recorded deliberately:** Brave sends GPC by default and Firefox does
with one toggle, so crashes from those users are now invisible to us —
Brave-Shields-specific breakage in particular is the class we are least likely
to hear about. That is bounded (a bug is not per-user; one unshielded reporter
surfaces it) and it was a choice, not a forced one: GPC concerns sale/sharing
rather than error reports, so softening the `/privacy` copy would have been
defensible too. We chose one honest switch over a caveated one.

Unchanged: the DSN gate, `sendDefaultPii: false`, the `beforeSend` scrubber, and
fork-silence. Native shells have no GPC/DNT, so only the cookie applies there.
