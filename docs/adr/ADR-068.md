...
**Draft — to be Accepted on first observability PR merge.** Implementation in Slice 2 of RFC-025 (~1 day). Slice 2 is silent-by-default, validated by `docker compose --profile observability up -d` + `docker exec grafana-agent ss -tn | wc -l` returning zero established connections when env vars are empty. Once the VPS RFC ships, this ADR is revisited only if the dashboards need restructuring; the integration shape itself is stable.
