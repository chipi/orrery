...
## Amendment — 2026-09-11 (the cookie could not say WHICH tour)

The schema above (`{ep, pos, idx, cmp, spd?, cc?}`) carried no tour identity, and
`AudioOverlay.acceptResume()` therefore rebuilt the queue from `CURATOR_FULL_TOUR`
unconditionally. Resuming the **extended** tour replayed the full tour's
sequence, and after ADR-093 added `tour-complete`, an exhibit-kiosk resume would
also have reported against the wrong tour.

Added an optional **`tid`** field (`'curator-full' | 'curator-extended' |
'exhibit'`, ≤32 chars). Optional for backwards compatibility: a cookie written
before this resumes as the full tour, which is exactly what the old code did, so
no existing resume changes behaviour. `acceptResume()` now maps the id back to
its sequence and passes it to `resumeTour()`, so the completion event reports the
tour that actually ran. An unrecognised id falls back to the full tour rather
than discarding the user's saved position.

Found during the ADR-092/093 pre-push review and fixed separately from it.
