...
- **Moon regional context = Kaguya TC (#361):** the Moon's CTX-equivalent Tier 2a layer. 12 detail-only /moon landers (Luna 9/16/17/21/24, Chang'e 3/4/5/6, Chandrayaan-3, SLIM, Beresheet) had no regional patch, so their selection bracket framed bare regolith. `scripts/hotspots/fetch-moon-kaguya-regional.ts` STAC-searches the USGS Astrogeology ARD Kaguya (SELENE) Terrain Camera collections and `gdal /vsicurl/` window-crops a ~16 km patch (~6–12 m/px) from the public S3 COGs — JAXA imagery, no full downloads. Self-credited under `JAXA-OPEN` with `instrument:'Kaguya TC'` → `/credits` **JAXA · Kaguya TC** section. Wiring the regional source also re-sizes the selection bracket to the 3.0u regional patch (fixes the over-sized bracket on these sites).
- **Surface select-landing + reveal (#361):** selecting a site lands at camR 37 (was 50, which dropped you before the regional ramp started); regional reveal `44 → 33`, detail `33 → 30.32`. Regional context is bracket-framed and ~64% revealed on arrival, sharpening to detail as you lean in.
