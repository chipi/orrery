...
- **The phase-scene contract** — `src/lib/three/flight-phase-scene.ts`: `interface FlightPhaseScene<TState>` (`scene`/`camera`/`setState`/`setAspect`/`setForceVisible`/`setForcesVisible`/`snapCamera`/`reset`/`dispose`) + `type ForceKey`. `AscentScene` implements it; the broadcast composer `ascent-renderer.ts` drives any `FlightPhaseScene`. **This is the seam a future descent/landing scene extends** (RFC-034 §9.1) — same interface, same composer, thrust-reversed physics, `Phase` union widened to add `'arrival'|'descent'`.
