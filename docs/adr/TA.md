...
|---|---|---|
| `/` | Sectioned discovery grid — 22 leaf cards in 5 nav-mirroring sections (EXPLORE · WORLDS · PLAN & FLY · CATALOG · LEARN), hero CTA + about prose | PRD-013 / RFC-038 / IA.md §home-cards |
...
| `/earth` | Hybrid scene — default orbital mode (ISS, Tiangong, Hubble, JWST, GNSS constellations) + surface mode (`?mode=surface`) with 26 launchpads via shared `SurfaceScene` — all with the Tier-2 zoom ladder, 6 with Tier-3 ground panoramas (#546). Mode toggle top-center. | PRD-005 / ADR-046 / ADR-072 / #285 / #546 |
...
**Tier-1 Earth-orbit re-entry — the *return* bookend (RFC-034 §13, 2026-07-19).** The ~31 Earth-orbit crewed capsules (Mercury/Gemini/Vostok/Voskhod/Apollo-CM/Skylab/Soyuz/Dragon/Shenzhou) fly **pad → orbit-coast → re-entry → recovery** on the *same* unified `/fly` timeline. `DescentBody` gains `'earth'` + an `EARTH_CAPSULE_REENTRY` archetype (ballistic → drogue → mains → splashdown/ground; a fouled main = honest IMPACT). The orbit-coast act (`fly-leo-coast-scene.ts` + `CoastScene.svelte`, km-unit Earth-centred) renders the capsule looping its ring with the **hybrid loop rule** — `min(realRevs, 3)` loops rendered, real REV/date counters carrying the scale (1 orbit → 1214). It's the `cruise` act of the **unified master scrubber**: `CoastScene` is `externalClock`-driven, the one scrubber drags continuously launch→coast→descent (`onMasterScrub` routes the cruise band to the coast; the rAF advances `coastMetDays`). 8 capsule families in `capsule-models.ts` (hero-PBR, **8 distinct hulls** — as of 2026-07-29 Voskhod carries its own soft-landing retro package vs Vostok's bare sphere, and Shenzhou is its own broad silver bell, not a scaled Soyuz), coast descriptors in `earth-orbit-registry.ts`, the Earth-return capsule profiles in `descent-profiles/` (incl. apollo8's lunar-return skip), launchers `mercury-redstone`/`voskhod-11a57`/`soyuz`/`long-march-2f`. **The same capsule mesh is now reused across all three acts** (2026-07-29): ascent (`ascent-scene.ts` `buildPayload` resolves a crewed `mission.id → capsuleId` and tags `userData.isCapsule`, so the scene rides the capsule **exposed** — no fairing, stowed shroud, or solar-wing deploy — then separates it at SECO), the orbit-coast, and the EDL descent (`descent-models.ts` `buildCapsuleById`). Before this, crewed ascents wrongly showed the generic solar-panel satellite bus. Surfaced in a dedicated **Human spaceflight capsules** section on `/colophon` (`build-original-work.mjs` `capsules` manifest key + static thumbs). Open follow-ups (descent-playback time-compression, in-scene re-entry HUD): `docs/wip/2026-07-19-tier1-earth-orbit-reentry.md`.
...
**Canonical-card aliases** — entities that exist on multiple surfaces share ONE card: fleet entries whose id is a mission (`fleetAliasesMission`, 47), surface sites matching a mission by `mission_id` or id parity (`siteAliasMissionId`, 45 of the 54 moon+mars sites; venus-sites.json is not enumerated by the generator/stubs — its 3 sites all alias missions by id parity and inherit mission cards, so a future non-aliasing Venus site would need wiring), and earth-objects (`earthObjectCardAlias`: lunar orbiters → mission, guarded by dest↔body so the Galileo GNSS constellation never aliases the Galileo Jupiter mission; constellations/stations/observatories → their `fleet_refs` fleet entry; only the GEO belt marker has no card). Aliasing panels fetch the **canonical entity's own gallery** for the hero so the in-app card is pixel-identical to the generated one.
...
- **Launch-pad tier ladder (#546):** all 26 launch pads carry the moon/mars Tier-2 zoom ladder — per-pad 512 m detail (USGS NAIP for US pads ~0.5 m/px; IGN BD ORTHO for Kourou; GSI seamlessphoto for Tanegashima; disclosed Sentinel-2 2.56 km coarse fallback where no open sub-meter source exists) + 16 km Sentinel-2 regional for every pad. Pads join `surface-hotspots.json` via `surfaceHotspotsSidecar()` merged in `earth-launch-site-adapter.ts`; the generic `launchpad-generic` Tier-1 builder (`register-earth-hotspot-builders.ts`) unlocks dispatcher enrolment. Six pads with an open-licensed ground photo additionally carry a Tier-3 panorama (`fetch-earth-panoramas.ts`; strip centred at texture azimuth 270° = the skybox's yaw-0 facing). Fetchers: `scripts/hotspots/fetch-earth-pads.ts` / `fetch-earth-panoramas.ts` (see `scripts/hotspots/README.md`). New `/credits` sections: USGS NAIP, Copernicus Sentinel-2, IGN, GSI.
...
- **Night-sky content layers (RFC-041 / #487–#488, Aug 2026)** — one sky, everything on it. On top of the Sun/Moon/planet/station reticles, `ar/sky-scene.ts` overlays **constellation figures + names**, **bright named stars**, **deep-sky objects** (nebulae/galaxies/clusters), a **horizon ring** with **sunrise/sunset markers**, and a **hide-below-horizon** default — all through the *same* equatorial-XYZ → `equatorialToHorizontal` → `skyDirectionENU` pipeline the planets use (pure math + loaders in `ar/celestial-sky.ts`, unit-tested; the celestial data is the existing `static/data/universe/*`). HUD layer toggles (Planets · Stars · Figures · Nebulae · Rise/set · Stations · All-names · Below-horizon) live in `launch-ar.ts`; everything on by default. **Editorial redesign (advisor-led):** box-free tracked shadow-halo labels in tiers, a label budget + screen-space declutter (priority + overlap + edge-fit), hard-core stars w/ diffraction spikes, shaped ice-blue deep-sky glows, a glass HUD strip. **Planets are baked from the shipped `static/textures/2k_*.jpg` maps** (`ar/planet-bake.ts`): each body rendered once as a real 3D sphere lit along its true Sun direction into a `WebGLRenderTarget` → sprite map (real albedo, Jupiter's belts + Red Spot, **Saturn's rings**, correct **Mercury/Venus/Moon phases**, granulated Sun; procedural canvas markers are the pre-bake fallback). Marker **size tracks apparent angular diameter** (log-compressed from live `distanceAu`), **brightness → a per-body aura**, phase → real Sun geometry (one quantity per channel). Stations render as **line-art spacecraft glyphs** (ISS truss + wings, Tiangong T + panels). Desktop-verified via the `/dev/sky-preview` harness (a mock `SkyView` at a fixed location/heading); on-device is the smoke test. Same coverage policy as the rest of `ar/` (excluded — WebGL/render-targets can't run in jsdom; the pure `celestial-sky.ts` keeps its `*.test.ts`).
...
**Caveats (honest limits).** Station TLEs live in the served /data overlay `static/data/station-tles.json`, refreshed from Celestrak by the single server-side fetcher `scripts/fetch-station-tles.mjs` — daily on main (`refresh-station-tles.yml`, bot-commit) and every 6h on the prod VPS (`ops/refresh-prod-data.sh`, live overlay, no redeploy), the same pipeline launches use (#404 · H4c #464). The app resolves the fresh served copy at runtime via `resolveStationTle`/`resolveStationTleBlock` (tle-source.ts) — **no browser→Celestrak egress** — and falls back to the build-baked import (`stations.ts`) when offline; the MCP image carries the baked copy. Propagation is J2-secular, not SGP4, and iss-pass discloses `epochAgeDays` vs `staleAfterDays` (H5) so a lagging container is honest about its age rather than silently wrong. Sky-pointing works cross-platform (ARKit heading-aligned, WebXR + compass correction, or the magic window), but the compass sign/offset conventions vary by OS/browser and can't be verified in jsdom — `sky-orientation.ts` exposes `AZIMUTH_CALIBRATION_RAD` as the single knob to nudge if a real device reads the sky rotated/mirrored. The magic window depends on a device magnetometer (uncalibrated compasses drift). **Platform parity:** the Android manifest carries `CAMERA` (#214) + `ACCESS_COARSE_LOCATION` (mirrors iOS `NSCameraUsageDescription` + `NSLocationWhenInUseUsageDescription`); gyro/compass needs no manifest entry on either OS (JS permission APIs — iOS prompts via `DeviceOrientationEvent.requestPermission`).
