...
> **Status:** Accepted (foundation v0.3.x — Paraglide + 12 locales live; RFC-010 closed via ADR-031 / ADR-032 / ADR-033; per-screen content rollout ongoing)
> **Depends on:** RFC-010 (closed via ADR-031 / ADR-032 / ADR-033)
> **Slice:** v0.3.x foundation; rolling
...
## §close — v0.7.0 a11y subset (2026-05-24)

Shipped via GH #256 (subset of the full Tier 3 flagship in #123). Concrete deliverables:

- **(A) Axe-core CI gate** — `tests/e2e/a11y.spec.ts` promoted from pilot (5 routes, logging only) → fails on `critical`-impact violations across 11 routes: `/` `/missions` `/fly` `/science` `/library` `/explore` `/earth` `/moon` `/mars` `/iss` `/tiangong`. Triage pass found ZERO criticals, only `serious` color-contrast warnings (mostly editorial dim text).
- **(B) Reduced-motion guardrail** — NEW `src/lib/reduced-motion-guardrail.test.ts` bans `transition:fly/fade/slide/scale/blur/draw` in `src/**/*.svelte` so a future commit can't re-introduce one that bypasses `prefers-reduced-motion`.
- **(C) Non-visual parallel mode for 6 canvas routes** — screen-reader-only `<ul>` mirroring each canvas's site-marker / module / body list. `<button>` per item fires the same panel-open handler as a canvas click. `/missions /tiangong /moon /mars /iss /earth`. `/fly + /explore` permanently out-of-scope (canvas-aria-label is honest for inherently visual simulation routes). `/missions` got `<dl>` semantics on the existing card metadata.
- **(V min) Locale-aware alt-text accessor** — NEW `src/lib/image-alt.ts` with `getImageAlt(path, locale)` + en-US baseline auto-generated from `image-provenance.json` (3210 entries). When a translator drops in `image-alt-text/<locale>.json` for any locale, screen readers in that locale immediately pick up the localized text — no code change needed.
- **(Z subset) Automated RTL probe** — NEW `tests/e2e/a11y-rtl-probe.spec.ts` asserts `<html dir="rtl">` on `ar` locale across `/missions /moon /library`. LTR-on-en-US sanity check too.

**Open follow-ups** (intentionally deferred):
- **Y narrated /fly playback** → v0.8 alongside audio narration epic (#146, #211, #219).
- **Z full RTL sweep** — needs native Arabic-speaking tester with VoiceOver / NVDA / TalkBack on real devices. v0.7.1+ (or whenever a tester is arranged).
- **V full caption pipeline** → GH #257 v0.9 (schema split, Argos pipeline integration, 1400 strings × 14 locales fanout).
- **X for /fly + /explore parallel mode** → dropped forever. Not deferred — explicit non-goal per memory `feedback_no_fly_explore_parallel_mode.md`.

**Lighthouse a11y target raised**: 100 on every covered route (was 'aspirational'); axe-core gate codifies it.
