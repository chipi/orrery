...
> **Status:** Accepted (v0.7 ship — surface hotspots on /moon, /mars; RFC-017 closed via ADR-059 / ADR-060 / ADR-061 / ADR-062)
...
### v0.7.x — full immersive exploration of each landing site

Refinements that landed (or land progressively) as v0.7.x patch increments:

- **Two-layer Tier 2 surface patches.** The single HiRISE / LROC NAC patch becomes a composite of (a) a wider *regional* layer showing landing-zone geological context (~10-100 km of ground) and (b) the existing *detail* layer (~0.5-2 km, lander-level). For Mars the regional source is Caltech Murray Lab's Global CTX Mosaic V01 (5 m/px, seamless, cite Dickson et al. 2024). For Moon the regional source is CNSA's Chang'e 2 lunar mosaic (7 m/px, higher resolution than LROC WAC's 100 m/px — the premium choice for Moon's regional layer, licensed CC-BY-CNSA). Detail layer for Moon remains LROC NAC at 50 cm/px. The two layers render as one Tier 2 unit; visual hierarchy comes from disc size + polygonOffset (detail above regional), no cross-fade between them. **Operator + maintainer playbook** (per-site sidecar fields, operator-override pins, fail-fast tuning, validation runbook, failure-mode → fix matrix) lives in [`docs/guides/mars-hotspot-imagery.md`](../guides/mars-hotspot-imagery.md). Two critical bugs were found and fixed in the crop pipeline during the 2026-05-21 audit — UInt16→UInt8 byte interleave and Polar_Stereographic projection misapplication — both documented in that guide; GitHub issue [#248](https://github.com/chipi/orrery/issues/248) carries the retrospective.

- **Contextual source-attribution info card.** When the camera enters the Tier 2 zoom band, a small floating card fades in showing (a) the site name + nation + mission context and (b) the currently dominant imagery layer's source + resolution + product ID. Card content transitions as the user zooms from regional → detail (e.g. "Curiosity · USA / Regional view · CTX Murray Lab mosaic / NASA·JPL·Caltech·MSSS / 5 m/px" → "Detail view · HiRISE ESP_030313_1755 / NASA·JPL·UAhirise / 25 cm/px"). Product ID links to the canonical agency page. Honest disclosure of where every pixel came from, in-context with the imagery itself rather than only in the Credits page.

- **Photo gallery side panel — multi-agency human + hardware narrative.** A new "Photos" tab in the existing site detail panel surfaces a curated grid of 3-5 stills per site: launch photo, hardware in clean room, crew portrait or mission patch, sample-return image, surface selfie or first photo from the lander. Click opens a fullscreen lightbox with caption + credit. Per-site sources span agencies — Apollo sites get NASA archival shots **and** Chang'e 2 lunar mosaic context (Chinese imagery of American landing sites, an honest multi-nation perspective). Soviet Mars 3 and Luna sites get Roscosmos archival material. Schiaparelli + Beagle 2 get ESA + UK Space Agency archives. License-allowlist updated per ADR-047 to accept CC-BY-CNSA / CC-BY-Roscosmos / CC-BY-ESA / CC-BY-ISRO / CC-BY-JAXA / CC-BY-SpaceIL where applicable.

These three additions compose into the "full immersive exploration" experience: see the landing region in geological context (regional Tier 2), see the lander hardware at the actual spot (detail Tier 2), stand at the lander vantage in 360° (Tier 3 panorama for 5 showcase sites), and browse the full hardware-+-human story across agencies (photo gallery for all 31 sites). Honest about where the imagery comes from at every step.

**Status of v0.7.x work blocks (2026-05-22):**

- **#PA** (auto Tier B fetch) ✓ — HiRISE catalog-driven auto-pick + GDAL crop, all 13 Mars sites covered.
- **#PB** (Mars CTX two-layer) ✓ — Murray Lab regional + HiRISE detail; both layers shipped + info card.
- **#PD-mars** (Mars Tier 3 panoramas) ✓ — **shipped multi-agency from launch** (originally scoped as 5 NASA showcases, expanded to 10 sites covering all available Mars surface panoramas + Zhurong/CNSA in the same slice). Composition is cylindrical-to-equirectangular padding with Mars sky/regolith fill + recolour-black for source data holes; renderer reuses the proven /moon skybox component. The CNSA-EDU fair-use license-allowlist entry (added under #PD-mars) is the first non-Western-agency precedent in the repo, expected to also cover Chang'e Moon imagery in #PC.
- **#PE-mars** (photo gallery) ✓ — multi-agency side panel for all 31 sites (13 Mars + 18 Moon); per-locale overlay path for STORY tab i18n.
- **#PC** (Moon Tier 2 + Tier 3 starter) ✓ — Phase 1 shipped: 17/18 Moon sites have a Tier 2 LROC patch (6 via BDR NAC_ROI mosaics for Apollo + 11 via Phase 2.5 hand-curated Featured Image JPEGs; luna9 deferred until LROC team locates the 1966 site). Tier 3 starter: 4 sites (apollo11, apollo14, apollo17, change4 — first farside surface panorama ever published, CC-BY-4.0). The two-layer "Chang'e 2 regional + LROC NAC detail" goal is Phase 2 (regional layer slot wired but empty; needs Chang'e 2 mosaic tile-server research per RFC-017 OQ-4). Two big findings during shipping: BDR NAC_ROIs are organised by *geographic feature* not landing-site coords, so they cover Apollo only (lessons in [docs/guides/moon-hotspot-imagery.md](../guides/moon-hotspot-imagery.md) §Two sourcing paths); LROC NAC BDR products are Float32 reflectance, the pre-existing HiRISE UInt16-only stretch path zeroed every Moon pixel until a Float32 branch landed (gdal-crop.ts).
- **#PF** (non-NASA missions, fleet, orbital trajectories) — last.

### v0.7.x — global space programs, not just NASA

Across every tier of every site — Tier 1 models, Tier 2 regional + detail patches, Tier 3 panoramas, photo galleries — actively surface non-NASA imagery and mission data where it makes the experience BETTER. The editorial purpose is honest multi-agency representation, but the operating principle is **best source per site for the user**, not "every agency on every site". Some sites are genuinely best served by one source (HiRISE for Mars detail, Chang'e 4 for the lunar far side); some sites are best as composites (Chang'e 2 regional context + LROC NAC detail at Apollo 11 — Chinese view of the American landing, an honest multi-perspective moment). UX-driven curation, not quota-filling.

Concrete additions in scope:

- **Tier 2 imagery beyond NASA.** Mars regional patch uses Caltech Murray Lab (NASA-funded, US). Moon regional patch uses **Chinese (CNSA) Chang'e 2 mosaic at 7 m/px** — chosen explicitly over LROC WAC because Chinese imagery is sharper for this layer. Investigate Tianwen-1 HiRIC, Chandrayaan-2 OHRC, SELENE/Kaguya Terrain Camera, Indian / Japanese / Soviet historic imagery as supplementary regional or detail sources per site.

- **Tier 3 panoramas beyond the 5 NASA showcases.** Original v0.7 scope was 5 Apollo / Mars-rover panoramas. v0.7.x extends to include first-of-its-kind milestones from every agency:
  - **Chang'e 3 + Yutu** (CNSA, Mare Imbrium 2013) — Chinese surface 360° panoramas.
  - **Chang'e 4 + Yutu-2** (CNSA, far side 2019) — *unique*: no NASA equivalent exists for the lunar far side, where Earth is never visible.
  - **Chang'e 5** (CNSA, sample-return Mons Rümker 2020) — first non-USA sample return from the Moon since 1976.
  - **Tianwen-1 + Zhurong** (CNSA, Utopia Planitia 2021) — Chinese Mars rover panoramas.
  - **Chandrayaan-3 + Pragyan** (ISRO, lunar south pole 2023) — India's first soft landing, highest-latitude landing site on the Moon.
  - **SLIM + LEV-1/2** (JAXA, Shioli crater 2024) — Japan's precision-landing demonstration, the upside-down lander.
  - **Luna 9** (USSR, Oceanus Procellarum 1966) — first soft landing ever, original Soviet panoramas in monochrome. Historic.
  - **Luna 17 + Lunokhod 1** (USSR, Mare Imbrium 1970) — first wheeled rover on any other body. Soviet ground-view panoramas.
  - **Schiaparelli** (ESA, 2016 crash) — debris analysis imagery, an honest "failed but on the record" treatment.
  - **Beresheet** (SpaceIL, 2019 crash) — Israeli private-mission attempt, distinct cultural milestone.

- **Photo gallery as global narrative.** Each site's photo grid (PRD-014 §v0.7.x above) deliberately mixes agency sources. Apollo 11's gallery includes Chinese Chang'e 2 high-res context of Tranquillity Base alongside NASA archival hardware shots. Mars 3's gallery includes original Soviet hardware photography. Chandrayaan-3's includes ISRO + foreign-news coverage. The lightbox credit string makes the source agency visible per image.

- **Missions index + Fleet expansion (#PF in work-block sequence).** All non-NASA surface missions surfaced in `/missions` and `/fleet` with the same care as NASA missions: launch date, vehicle (Long March 3B, Soyuz, GSLV Mk III, H-IIA, Falcon 9, etc.), agency, mission summary, outcome, science objectives. Plot their orbital trajectories on `/moon` and `/mars` where flown — Luna sample-return trajectories, Chang'e Earth-Moon transfer orbits, Tianwen-1 Mars insertion, Chandrayaan-3 trans-lunar injection. Same care, same provenance discipline. The story of space exploration is human, not American.

License-allowlist entries needed (per ADR-047, additions on top of existing PD-NASA / PD-Russia):
- `CNSA-EDU` ✓ — **landed in v0.7.x #PD-mars**. China National Space Administration does not publish a formal Creative Commons licence for Tianwen-1 / Zhurong imagery, but its standard practice is editorial / educational reuse with credit (Planetary Society, academic journals, Xinhua redistribution). Orrery embeds with the canonical "CNSA / PEC" attribution + a documented fair-use rationale. Will also cover Chang'e Moon imagery under #PC. The originally-planned `CC-BY-CNSA` tag turned out not to exist as a real CNSA policy; `CNSA-EDU` is the honest replacement.
- `CC-BY-Roscosmos` — Russian Federal Space Agency
- `CC-BY-ESA` — European Space Agency permissive terms
- `CC-BY-ISRO` — Indian Space Research Organisation
- `CC-BY-JAXA` — Japan Aerospace Exploration Agency
- `CC-BY-SpaceIL` — Israeli private mission

Note: where a non-Western-agency image is also redistributed under a Creative Commons licence (e.g. Zhurong's pre-egress panorama appears as CC-BY-4.0 in a 2024 academic publication on Wikimedia Commons), prefer the CC tag over the agency-EDU fallback. The `CNSA-EDU` allowlist entry is the safety net for direct-from-CNSA content that doesn't have such redistribution.
...
Surface Hotspots roadmap shipped end-to-end across the #PA–#PF wave. All 36 Mars + Moon landing sites render through the 4-tier zoom-aware system: Tier 0 silhouette → Tier 1 3D model → Tier 2 LROC NAC / HiRISE patch → Tier 3 equirectangular skybox panorama via "Stand at site".

**#PA-#PF summary**:
- **#PA** Tier B Hybrid automated LROC/HiRISE patch fetch via `scripts/hotspots/lroc-fetch.ts` etc.
- **#PB** Mars CTX regional layer + two-layer Tier 2 + info card; docs refreshed.
- **#PC** Moon LROC two-layer Tier 2; Chang'e 2 premium regional research closed (RFC-017 OQ-4 — operator-manual ingest path since CNSA portal has no programmatic API).
- **#PD-mars** Mars Tier 3 panoramas: 10 NASA + 1 CNSA shipped Mars-first.
- **#PE** Multi-agency photo gallery side-panel across all sites (CNSA / ISRO / JAXA / Roscosmos / ESA / SpaceIL alongside NASA).
- **#PF** Non-NASA missions / fleet / orbital trajectories: 4 missing cislunar_profile entries filled (change3 / luna16 / luna21 / beresheet); 8 single-image fleet entries closed via Wikipedia REST API curation; trans-Mars trajectory overlay rendered on /fly via new `src/lib/historical-mars-arcs.ts`.

**Tier 3 panorama coverage (Moon)**: 18/18 sites. Where a true 360° pan exists (Apollo 11-17, Chang'e 3, Lunokhod 1) it's used directly. Where only narrower coverage exists (Chang'e 5/6) it pads to equirectangular. Where only a single still or descent frame exists (Chandrayaan-3, SLIM, Luna 16/24, Beresheet) it ships with honest editorial captioning matching the mars3 / beagle2 / schiaparelli pattern. License-allowlist expanded: CNSA-EDU, ISRO-EDU, JAXA-OPEN, SpaceIL-EDU, PD-Russia.

**Operational artefacts**:
- `scripts/audit-gallery-counts.ts` (--non-nasa filter) — flags gallery gaps across fleet + missions.
- `scripts/rebuild-gallery-manifests.ts` — repairs stale fleet/mission-galleries.json from disk reality.
- `docs/guides/moon-hotspot-imagery.md` — operator + maintainer playbook with per-agency source list.

**Open follow-ups** (intentionally deferred):
- ~~GH #107~~ — **CLOSED 2026-05-28.** Full corpus rollout shipped: 21 Moon + 14 Mars + 4 outer-system missions now carry `tier_1_5_hybrid` waypoints + phase markers + science chips + click-to-jump scrubber on `/fly`. Tier 2 (real NASA TND state-vector ingest) reserved for v0.8+ as GH #262. See ADR-058 third amendment + `docs/guides/mission-trajectories.md`.
- GH #255 — Trans-Mars overlay extension: agency filter UI, /mars route rendering. v0.7.1+ candidate.
- 9 non-NASA fleet entries still under 3-image threshold (spacesuit + observatory specialty hardware) — genuinely hard to source from public archives.
