...
1. **Bundle-size strategy.** Naive `npx cap sync` copies all 355 MB of `build/` into both `android/app/src/main/assets/public/` and `ios/App/App/public/`. iOS App Store has a 200 MB OTA cellular-download cap (4 GB hard cap). Android Play has no hard cap but ratings drop visibly above ~150 MB. The asset bulk is fleet galleries (~120 MB), science diagrams (~10 MB), agency logos (~3 MB), planet textures (~30 MB), fonts (~5 MB), 14-locale message bundles + JSON overlays (~150 MB). RFC-018 § bundle-strategy proposes: **bundle the shell + minimal assets (~50 MB) + lazy-fetch fleet galleries from `chipi.github.io` on first view per category**. This trades pure offline for distribution viability. Decide before the first Internal Testing build.

2. **Data-update mechanism.** The web app's PWA service worker now operates in `autoUpdate` mode (May 2026 refresh) — new bundles install silently on next navigation. In the Capacitor WebView, the same SW runs and the same autoUpdate semantics apply for any data fetched from `chipi.github.io`. Bundled data (whatever ships inside the .aab / .ipa) updates only on app-version submission. Is the answer "bundle a snapshot, fetch deltas via SW"? Resolved alongside Q1.

3. **Tablet — Android tablet + iPad.** Desktop layout fits tablets reasonably well — `/explore`, `/missions`, `/science` all read fine at 1024×1366. Does it ship as-is, or get a dedicated tablet breakpoint? Decide after first physical-device testing on both platforms.

4. **App icon design.** The Bebas Neue wordmark doesn't translate to a 60×60 px home-screen icon. Options:
...
5. **Internal Testing cohorts.** Need at least 5 Android testers (range: Pixel through Galaxy mid-range) and 5 iOS testers (range: SE through Pro Max) with real devices.

6. **Code signing.**
   - **Android:** Play App Signing (Google manages the upload key); only the upload key needs to be in our control. Operationally simple.
   - **iOS:** Apple Developer Programme membership + provisioning profiles required. Open ownership question: who manages signing certs?
