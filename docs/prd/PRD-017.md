...
> **⚠ As-built reframe (2026-07-11).** The v0.8 build replanned this to a "good
> simple" cross-platform scope — treat `docs/adr/TA.md §sensory` +
> `docs/wip/sensory-cue-vocabulary.md` as authoritative. Kept: full gyroscope,
> master-toggle + settings sheet, opt-in/in-memory, narration ducking. Changed:
> the 11-route sonification (promises 1–12) shrank to **2 hero voices** (Kepler
> chord + /fly velocity); haptics + discrete sonification merged into a paired
> **event-cue tier** (`select` wired app-wide; confirm/threshold/warning defined
> but untriggered in today's app). Dropped: the 5 new-route sonification designs,
> the recalibrate gesture, per-route listening-review gates.
