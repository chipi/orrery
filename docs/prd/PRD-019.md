...
> **⚠ Amendment v1.1 (2026-07 · #393/#402–#408).** Implementation expanded past the original v1 scope below. Three things this PRD still marks as deferred/absent have SHIPPED — treat **[TA.md §immersive](../adr/TA.md)** as authoritative where they disagree:
> 1. **`/iss` + `/tiangong` station tabletop AR shipped in v1** (#407 + #408 assembly replay), not v2 — the procedural proxy models made tabletop reuse cheap. Supersedes the "defer to v2" non-goal + M4.
> 2. **AR-astronomy modes added** (beyond the original 4-globe tap-to-place scope): sky-pointing (#393), real-now Earth/Explore (#402/#403), stations orbiting the real Earth + marked in the sky (#405/#406), backed by new `$lib/astronomy` + `$lib/satellite` engines.
> 3. **iPhone Safari is NOT AR-less after all.** The sky-pointing "magic window" (rear-camera feed + DeviceOrientation compass) runs in iPhone Safari and non-ARCore Android — so the `SKY` button is *enabled* there, not greyed. Only tap-to-place tabletop AR stays wrapped-app/WebXR-only. Supersedes the "iPhone Safari: no AR" claims below.
...
| **M15** | **ARKit Swift plugin scope.** `@orrery/ar-bridge` Capacitor plugin (Swift package) exposes 5 Capacitor-callable methods: `requestSession()` / `endSession()` / `hitTest(x, y)` / `addAnchor(anchor)` / `removeAnchor(anchor)`. `getCameraPose()` is JS-local — reads a pose cached from the per-frame event. Plus 3 events emitted to JS: `session-started`, `frame` (per RAF with camera pose), `session-ended`. ~316 lines Swift (as shipped). Lives in the Capacitor `ios/` directory; no separate iOS repo. |
...
- New `@orrery/ar-bridge` Capacitor plugin, ~316 lines Swift (as shipped).
...
9. **ARKit Swift plugin maintenance burden.** ~316 lines of Swift (as shipped) + Apple's ARKit API evolution. Who owns it long-term? Marko (per PRD-015 iOS signing) is the default — confirm acceptable maintenance commitment.
