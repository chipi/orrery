...
- `scripts/mobile/add-mobile-rung.mjs` — generated the ≤1280 rung for exactly the 141 over-serve images by downscaling the **served base** (the buckets have no masters / only LFS stubs; one lanczos step avoids the full-corpus-regen / LFS-stub degrade the pipeline guards prevent). Additive, idempotent, diff-gated: 141 new `-1280.webp` + manifest entries, zero base images touched.
