...
> **Status:** Open
> **Author:** product
> **Closes into:** ADR-034 (math renderer), ADR-035 (diagram pipeline), ADR-036 (cross-screen `?`-chip pattern)
> **Slice gate:** v0.4
> **Why this is an RFC:** PRD-008 locks the *what* (in-app encyclopedia, six tabs, ~40 sections, ?-chip cross-link from existing screens). It does not lock the *how* — math rendering, diagram authoring, and the cross-screen UX of the `?` chip each have multiple defensible answers with real trade-offs. Each open question below has two-to-four options; we ship the resolution as ADRs alongside the implementation.
