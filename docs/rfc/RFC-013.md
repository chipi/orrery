...
**Recommendation: B for V1.** A static Earth limb is a 90% visual win for a 10% cost. We're already using `2k_earth_clouds.jpg` on `/explore` — same asset, no new fetch. Place it permanently below the station's centre of mass; the user always sees the station "above" Earth as in canonical photos.
...
- **ADR-040** — Asset pipeline: NASA-3D Resources GLB simplified to ~50k tris, committed to `static/models/iss/`. Earth backdrop using existing `2k_earth_clouds.jpg`. Nav slot after EARTH.
