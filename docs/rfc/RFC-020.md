...
*Status: Draft v0.4 · 2026-05-16 · **partly superseded by the v0.8 as-built reframe (2026-07-11)** · Closes: PRD-017*

> **⚠ As-built correction (2026-07-11).** After the mobile wrapper shipped we
> replanned this layer to a "good simple" cross-platform scope. What actually
> shipped differs from the sections below — read `docs/adr/TA.md §sensory` (the
> as-built map) and `docs/wip/sensory-cue-vocabulary.md` (the cue taste-def) as
> authoritative. Key deltas: **gyroscope kept full** (all 7 scenes) via one shared
> `device-orientation.ts` service; **sonification cut from 11 bespoke per-route
> graphs to 2 hero voices** (/explore Kepler chord, /fly velocity); **haptics
> folded into a paired audio+haptic event-cue tier** (`feedback.ts` `cue()`) with a
> 4-cue vocabulary of which only `select` has natural triggers today; the settings
> sheet is **one nav button that opens it** (not a long-press master); the §3.3
> five-new-route designs, the §7.5 recalibrate gesture, and the per-route
> listening-review slices are dropped.
