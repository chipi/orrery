...
> **⚠ Amendment v1.1 (2026-07 · #393/#402–#408).** Shipped scope grew past the decisions below; **[TA.md §immersive](../adr/TA.md)** is authoritative where they diverge:
> - **S-E amended:** `/iss` + `/tiangong` station tabletop AR shipped in v1 (#407 + #408 assembly-on-placement replay), not deferred — reusing the procedural proxy models made it cheap. §24 open item is thereby resolved for the station part (`/fly` AU-scale AR stays deferred).
> - **AR-astronomy modes added** beyond the 4-globe tap-to-place scope: sky-pointing (#393) + real-now Earth/Explore (#402/#403) + station passes (#405/#406), on new `$lib/astronomy` + `$lib/satellite` engines.
> - **iPhone Safari AR is no longer empty:** the sky-pointing "magic window" (rear-camera + DeviceOrientation compass) works there and on non-ARCore Android, so `SKY` is enabled (not greyed). Tap-to-place tabletop AR stays wrapped-app/WebXR-only, so the greyed "Enter AR" fallback below still holds for THAT button.
...
~330 lines Swift (as shipped) in `ios/App/App/Plugins/ar-bridge/`:

- `ArBridgePlugin.swift` — Capacitor plugin entry; bridges JS calls to ARKit Swift API.
- `ArBridgePlugin.m` — Objective-C `CAP_PLUGIN` registration macro; Capacitor requires this to discover the plugin at runtime.
...
`?mode=exhibit` URL parameter on any flat-screen route. Loads a separate Vite lazy chunk (`src/lib/exhibit.svelte.ts`, <20 KB).
...
Total v1 ship bundle impact: ~88 KB gzipped of new JS, all lazy-loaded only when AR or Exhibit Mode is entered. Plus ~330 lines Swift (as shipped) (iOS bundle only).
...
24. **`/fly` mission arc in AR.** (`/iss` + `/tiangong` station-model AR — **RESOLVED, shipped in v1**: #407 tabletop + #408 assembly replay; see the v1.1 amendment. `/fly` AU-scale AR remains deferred.)
