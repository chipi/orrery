...
0. **BLOCKER — bulk re-encode vs the image-authorization system (found in Slice 1b, 2026-07-08).** `validate-image-dupes` + `validate-image-phash-dupes` are hand-curated authorization gates: a byte-dupe `ALLOWLIST` keyed on 8-char SHA-256 (17 dated `ALLOWLIST_AUTHORIZED` entries, no regen script) + a phash baseline. They scan the **derived** `static/images` tree. A bulk re-encode (Slice 1b cap; Slice 2 WebP) changes every image's SHA → invalidates the allowlist wholesale and merges near-dupe pairs into byte-identical (Slice 1b produced 140 un-allowlisted byte groups + 68 phash pairs). The validators **can't** simply repoint at `masters/` — those are LFS `fetchexclude`d, absent on CI. Candidate resolutions: **(a)** move dupe validation to a **local-only** check over `masters/` (the stable source of truth; dupe curation is an authoring concern, not a deploy gate — drop it from CI `validate-data`); **(b)** finish the #384 dupe cleanup first, then re-encode once and re-snapshot a clean baseline; **(c)** regenerate the allowlist on every re-encode (rejected — the hand-authorization intent). Leaning (a), coupled to #384. **Gates Slice 1b/2 re-encoding.**
