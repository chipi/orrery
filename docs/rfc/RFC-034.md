...
One shared gain set flies every adequately-powered **serial** stack to orbit — falcon-9, titan-ii-glv, saturn-ib, proton-k — with no per-vehicle tuning. The `flagship-profiles.test.ts` GENUINE_ORBIT tier is the regression guard.

**Deferred tracks (still lob / can't insert; tracked in #415):**
- **Track 2 — coast + restart.** A low-TWR (< 1) upper stage can't hold altitude on a continuous burn; it must coast engine-off to apoapsis then relight to circularise. Unlocks **atlas-v** (Centaur) and **saturn-v** (S-IVB — a partial burn to a park orbit, coast, then reignite for TLI), plus the Delta IV / Titan III–IV upper stages in #414.
