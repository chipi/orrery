...
- **Vehicle:** a stylised multi-stage body (procedural or a light GLB per flagship), stages that visibly separate, fairing that splits, engine plume that scales with throttle. Reuse the fleet 3D-model pipeline (RFC-016 / ADR-052) where a model exists.
