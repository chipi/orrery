...
- **D1 (re-opened):** TLEs — **T-none** (leave the build-baked bundle as the offline fallback behind the live Celestrak fetch; retire the TLE cron) vs **T1** (migrate the fallback to the overlay via a sync→async refactor). Rec: **T-none** — the live fetch already keeps prod fresh.
