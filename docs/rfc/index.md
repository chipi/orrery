...
| RFC-039 | The Known Universe — extend /explore's shell ladder to 8 tiers: frozen "add-a-shell" seam + supergalactic coords/schema + Milky-Way-first milestone + science-lens-everywhere + chip taxonomy + cosmology learning set | Draft (v0.9 epic kickoff) · implements PRD-034 | PRD-034 | Milestone 1 = level the Milky Way |
| RFC-040 | Mobile offline — the mobile image rung (complete the ≤1280 srcset rung for the buckets that lack it: hotspots, no-ladder galleries) + tiered offline caching (Cache-API pre-download, per-tier manifest, Settings section) | Draft · implements PRD-035 | PRD-035 | Part 1 = mobile rung green before Part 2 |
