...
| RFC-037 | Physics Kernel — technical architecture for the Lab subsystem + MCP server: 4 frozen contracts (kernel boundary `src/lib/physics/` + lint gate · `FigureSpec` draw-once · formula-registry/card serialization · authoritative-ephemeris + compute caps), data strategy, Notebook-first slice plan. Vision in PRD-033, UX in UXS-015 | Draft | #TBD | v0.9.x |
