...
| RFC-035 | Prod data freshness — move the launch/TLE refresh from GitHub Actions to an on-VPS cron (pipeline-runner → `/data` overlay, keyless, no rebuild); gitignore live data so `git reset --hard` + seed can't regress it; TLE overlay migration (T1); GitHub-Pages→docs-only | Draft · closes into RFC-024 | — | v0.8.x |
| RFC-036 | /fly + /explore god-file decomposition — thin pages over pure, tested controllers + `$lib` scene hosts (byte-identical behavior-preserving refactor); the seam-contract precedent for large-scene work | Shipped (v0.8; on prod) | — | #440 · #441 · #443 |
| RFC-037 | Physics Kernel — technical architecture for the Lab subsystem + MCP server: 4 frozen contracts (kernel boundary `src/lib/physics/` + lint gate · `FigureSpec` draw-once · formula-registry/card serialization · authoritative-ephemeris + compute caps), data strategy, Notebook-first slice plan. Vision in PRD-033, UX in UXS-015 | Draft (epic kickoff) | #458 | v0.9.x |
| RFC-039 | The Known Universe — extend /explore's shell ladder to 8 tiers: frozen "add-a-shell" seam + supergalactic coords/schema + Milky-Way-first milestone + science-lens-everywhere + chip taxonomy + cosmology learning set | Shipped (v0.8.0) · implements PRD-034 | PRD-034 | Delivered: 8-shell ladder + Milky-Way-first + cosmology lens |
| RFC-040 | Mobile offline — the mobile image rung (complete the ≤1280 srcset rung for the buckets that lack it: hotspots, no-ladder galleries) + tiered offline caching (Cache-API pre-download, per-tier manifest, Settings section) | Shipped (v0.8.0, leftover #482) · implements PRD-035 | PRD-035 | Delivered; leftover: 62 .jpg mobile-rung conversion (#482) |
