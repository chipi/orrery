...
> Related PRD · PRD-001
> Related ADRs · ADR-001 (Three.js r128), ADR-011 (TypeScript), ADR-012 (SvelteKit)
...
- Tab condition: GALLERY shown when `static/data/planet-galleries.json[id]` (or `sun-gallery.json`) > 0; LEARN shown when overlay has non-empty `links[]`.

### Tokens (extension)
**GALLERY thumb (planets, default):** `border-color rgba(68, 102, 255, 0.6)` on hover (Earth blue accent).
**GALLERY thumb (sun):** `border-color rgba(255, 200, 80, 0.6)` on hover (theme-gold override; scoped CSS in SunPanel).
**Active tab border (planets):** `#4466ff`. **Active tab border (sun):** `#ffc850`.
