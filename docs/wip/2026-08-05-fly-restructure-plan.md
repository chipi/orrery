...
#### 1b — frame body + draw2d: **LANDED (2026-08-06, uncommitted → checkpoint)**
- **Done:** the `onFrame` body + `draw2d` + their 2D/frame state are out of `onMount`, verbatim, in `$lib/three/fly-frame-runner.ts` (`createFlyFrameRunner(bridge, refs) → { frame }`). The page builds `frameBridge` (get/set accessors over 62 reactive vars — 46 `$state`/`let` get+set, 16 `$derived` getter-only) + `frameRefs` (57 scene refs/handles/updaters/fns), then `createAnimateLoop({ onFrame: frameRunner.frame })`. `+page.svelte` **8,649 → 6,814 lines** (−1,835); the runner module is ~2,000 lines.
- **Gates (all green):** `svelte-check` 0 errors / 3325 files · `/fly` e2e desktop-chromium **150 passed / 0 failed** (7.2m) · `test:coverage` **4794 passed**, 92.34/80.42/89.9/94.36, EXIT=0 (the 5 WS-B imperative modules — frame-runner, camera-controller, helio-reactive, cislunar-reactive, helio-mission — added to `vite.config.ts` coverage-exclude, same WebGL policy as `fly-helio-overlays`) · runtime smoke 120fps, helio+cislunar branches scrub clean, scene renders.
- **Boundary lessons hit (recorded for WS-C):** the blind `\bname\b → bridge.name` retarget mangled (a) object **shorthands** `{ reducedMotion }` → need `{ reducedMotion: bridge.reducedMotion }`, (b) **member-accesses** `cineOut.cutBlackOpacity` → `.bridge.` false-prefix, (c) **comments**, and (d) **function args on their own line** (a fix regex over-matched them into `key: value`). Also: the "Cannot find name" grep misses **shorthand-only** undefined deps (`cruiseHoldTriggerSimDay`) — they surface as "No value exists for the shorthand property". Fix-forward is reliable when driven strictly by typecheck errors, but the `any`-typed bridge/refs means typecheck does NOT gate the bucketing — e2e is the real gate.
- **1b left `onMount` at ~864 lines** (frame out; scene-assembly wiring + input handlers + bridge/refs block remained). **Closed by 1c below.**

#### 1c — input handlers + scene host: **LANDED (2026-08-06, uncommitted → checkpoint)** — `onMount` now **~559 lines (<600 ✓)**
- **`$lib/three/fly-input-handlers.ts`** (`createFlyInputHandlers(deps) → { get isDrag, get touchActive }`) — mouse orbit/pan + wheel zoom + one-/two-finger touch, verbatim. Owns its own drag/pinch state; the frame reads `flyInput.isDrag`/`.touchActive` via the bridge getters. −137 lines.
- **`$lib/three/fly-scene-host.ts`** (`createFlySceneHost(deps) → handle`, the B1 contract) — the whole 3D assembly (helio + cislunar scenes, both reactive overlay layers, per-mission overlays, spacecraft sprite + plume, cinematic camera controller), verbatim. 24 getter-closure deps in; ~50 refs out on the handle. The component-`$state` backflow a module can't write (DebugPanel `live*` passes, the mission-swap `let`s, the moon-mesh refs) stays in the page, assigned from the handle. −170 lines net.
- `+page.svelte` **6,814 → 6,496**; `onMount` **864 → ~559**. `svelte-check` 0 errors / 3326 files. Both new modules join the `vite.config` coverage-exclude (same WebGL policy). Runtime smoke: 120fps, Apollo (cislunar) + Perseverance (helio) scrub clean, scene renders, only the pre-existing trajectory-JSON 404.
- **(historical) Regions:** the `createAnimateLoop({ onFrame })` body = `+page.svelte` **3600–5069** (~1,470 lines) + `draw2d` **3096–3562** (~466 lines, the 2D-fallback renderer; its `c2`/`ctx2`/`cis2d*` state at 3077–3095 is self-contained — the frame never touches it) = **~1,940 lines**, larger than 1a.
