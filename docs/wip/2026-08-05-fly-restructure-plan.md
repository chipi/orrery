...
- **DEVIATION from B5:** `onMount` is now **~864 lines**, not the RFC's `<~600` target. The remaining bulk is scene-assembly wiring (`build*Handles`/reactive/mission/camera creation + destructures) + input handlers + the ~130-line bridge/refs block — separate concerns from the frame, and a further slice (extract the scene-assembly into `createFlySceneHost`) is what closes the last ~260 lines. 1b's own deliverable (pull the 1,465-line frame body out) is complete.
