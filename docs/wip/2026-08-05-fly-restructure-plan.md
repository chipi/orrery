...
#### 1a — camera subsystem: SCOPED, ready for a focused pass (not yet moved)
...
- **1b** (after 1a): the remaining frame body via the `$state` getter/setter bridge (the ~18 template-bound `$state` writes).
