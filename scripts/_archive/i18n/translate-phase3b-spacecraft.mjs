#!/usr/bin/env node
/**
 * Translate the Phase-3b spacecraft backfill overlays (Dawn, Giotto,
 * JUICE, BepiColombo, MESSENGER, Blue Moon Mk1, Clementine, Pioneer 11,
 * Ulysses, Vega 1, Vega 2, Venera 13) into all 13 non-English locales.
 * Pure data layer — no API calls; translations live inline so the file
 * is reviewable + diff-friendly. Same shape as translate-new-launchers.mjs
 * and translate-phase3a-launchers.mjs.
 *
 * Schema per (spacecraft, locale): { name?, tagline, description,
 * best_known_for }. Empty entries fall through to the en-US base.
 *
 * Run from project root:  node scripts/translate-phase3b-spacecraft.mjs
 */
...
async function main() {
  ...
  for (const [spacecraft, byLocale] of Object.entries(OVERLAYS)) {
    ...
    for (const locale of LOCALES) {
      ...
      if (!entry) {
        console.warn(`  ⚠ ${spacecraft}/${locale}: no inline translation, skipping`);
        continue;
      }
      const path = join(I18N_ROOT, locale, 'fleet', cat, `${spacecraft}.json`);
