#!/usr/bin/env node
// Fetch Wikimedia images for the 4 Slice A fleet fillers.
...
const SPEC = JSON.parse(readFileSync('/tmp/slice-a-fleet-images.json', 'utf8'));
...
for (const [entry, items] of Object.entries(SPEC)) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const url = `${W}/${encodeURIComponent(filename)}${isSvg ? '?width=1600' : '?width=1600'}`;
    try {
