#!/usr/bin/env node
// Fetch 5 Wikimedia images per Tier B entry (4 for change1 per Commons availability),
// generate 1×1/4×3/16×9 variants, register in fleet-image-sources.json.
// Mirrors scripts/fetch-phase3c-images.mjs.
...
const SPEC = JSON.parse(readFileSync('/tmp/tier-b-images.json', 'utf8'));
...
for (const [entry, items] of Object.entries(SPEC)) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      ...
      // register in fleet-image-sources
