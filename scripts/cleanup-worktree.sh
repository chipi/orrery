...
#      e.g. run from orrery-311982 -> ~/.treehouse/orrery*, so orrery-fixes is
#      swept but podcast is NOT). These leak when a session dies before its
#      SessionEnd reaper fires, orphaned in a sibling worktree nothing else targets
#      (seen: loops surviving 8-14 days). It reaches sibling worktrees but never
#      another project — the family + shape + age + CWD stack can't match a live
#      watch or an MCP server. Mirrors the auto sweep in ~/.claude/hooks/session-reap.sh.
#
...
# name up to its first '-'.  ~/.treehouse/orrery-311982/... & ~/.treehouse/orrery-
# fixes-311982/... both -> ~/.treehouse/orrery (one family); ~/Projects/podcast_
# scraper-x -> ~/Projects/podcast_scraper. Generic, not treehouse-specific.
family_prefix() {
...
# never other projects: run from orrery it reaps orrery-family loops only, never a
# podcast one. Reaps a `gh run …` call stuck in an unbounded while/until loop older
