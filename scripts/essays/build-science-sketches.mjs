...
const grain = (cx, cy, r) => {
  let seed = cx * 3 + cy; const rnd = () => (seed = (seed * 1103515245 + 12345) & 0x7fffffff) / 0x7fffffff;
  let pts = '';
  for (let a = 0; a < 360; a += 45) { const rr = r * (0.6 + rnd() * 0.7); const rad = (a * Math.PI) / 180; pts += `${(cx + rr * Math.cos(rad)).toFixed(0)},${(cy + rr * Math.sin(rad)).toFixed(0)} `; }
  return `<polygon points="${pts.trim()}" fill="rgba(127,176,224,0.14)" stroke="${LINE}" stroke-width="1.4"/>`;
};
const stars = (() => { let s = '', seed = 991; const rnd = () => (seed = (seed * 1103515245 + 12345) & 0x7fffffff) / 0x7fffffff;
  for (let i = 0; i < 70; i++) s += `<circle cx="${Math.round(rnd() * W)}" cy="${Math.round(rnd() * H)}" r="${(rnd() * 1.2 + 0.3).toFixed(2)}" fill="rgba(255,255,255,${(rnd() * 0.5 + 0.15).toFixed(2)})"/>`;
  return s; })();
...
const diagrams = {
  regolith: frame(`
    ...
    ${(() => { let g = ''; let seed = 5; const rnd = () => (seed = (seed * 1103515245 + 12345) & 0x7fffffff) / 0x7fffffff;
      for (let i = 0; i < 60; i++) g += grain(250 + rnd() * 700, 385 + rnd() * 190, 8 + rnd() * 14); return g; })()}
  ...
  'antimatter-propulsion': frame(`
    ...
    <g stroke="${GOLD}" stroke-width="1.8">${[0,45,90,135,180,225,270,315].map(a=>{const r=a*Math.PI/180;return `<line x1="460" y1="430" x2="${(460+90*Math.cos(r)).toFixed(0)}" y2="${(430+90*Math.sin(r)).toFixed(0)}"/>`;}).join('')}</g>
