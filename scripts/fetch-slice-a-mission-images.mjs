#!/usr/bin/env node
// Fetch Wikimedia images for the 4 Slice A mission galleries.
// Images go to static/images/mission-galleries/<id>/ with variants;
// mission-galleries.json id→count gets bumped.
...
const SPEC = JSON.parse(readFileSync('/tmp/slice-a-images.json', 'utf8'));
const SOURCES = JSON.parse(readFileSync('static/data/fleet-image-sources.json', 'utf8'));
const GAL = JSON.parse(readFileSync('static/data/mission-galleries.json', 'utf8'));

let ok = 0,
  fail = 0,
  skipped = 0;
for (const [entry, items] of Object.entries(SPEC)) {
  const dir = `static/images/mission-galleries/${entry}`;
...
writeFileSync('static/data/mission-galleries.json', JSON.stringify(GAL, null, 2) + '\n');
