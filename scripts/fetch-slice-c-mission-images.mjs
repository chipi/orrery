...
const SPEC = JSON.parse(readFileSync('/tmp/slice-c-images.json', 'utf8'));
...
// Mariner 10 goes into mercury/, OTV-* go into earth/.
...
for (const [entry, items] of Object.entries(SPEC)) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      ...
      ok++;
      console.log(`✓ ${entry}/${slot} (${agency}) [${DEST(entry)}]`);
