#!/usr/bin/env node
// Fetch Wikimedia images for Tier C entries that don't already have galleries
// on disk. viking2-lander + beagle2 are skipped — they already ship gallery
// images from earlier work. mars6 only has 2 files on Wikimedia Commons
// (the category is genuinely sparse) — we fetch what's there and bump the
// galleries count to match.
...
const ALL = JSON.parse(readFileSync('/tmp/tier-c-images.json', 'utf8'));
// Skip these two — already have on-disk galleries from earlier work.
const SKIP = new Set(['viking2-lander', 'beagle2']);
const SPEC = Object.fromEntries(Object.entries(ALL).filter(([k]) => !SKIP.has(k)));
...
for (const [entry, items] of Object.entries(SPEC)) {
  ...
  // Sync galleries count with actual files now on disk.
