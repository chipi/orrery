...
if (files.length !== numChunks) {
  console.error(`Expected ${numChunks} chunk-NN.txt files in ${combinedDir}, found ${files.length}`)
  process.exit(1)
}
...
for (let ci = 0; ci < numChunks; ci++) {
  ...
  for (const loc of LOCALES) {
    const hdr = `===${loc}===`
    if (lines[idx] !== hdr) {
      console.error(`${files[ci]}: expected "${hdr}" at line ${idx + 1}, got ${JSON.stringify(lines[idx])}`)
      process.exit(1)
    }
    idx++
