...
{#if stats && (inline || (layerOn && focusGate))}
  ...
  >
    ...
    <!-- Hero tile (#385 A) — the atmosphere's voice. The audio button is
         the single AT-exposed control; the eyebrow, instrument tiles, and
         numeric readout below are a decorative HUD duplicate of data
         available elsewhere, so they stay aria-hidden. -->
    <AtmosphereWaveform bodyKey={bodyLabel.toLowerCase()} />
    <div class="scan-decor" aria-hidden="true">
      <InstrumentTiles bodyKey={bodyLabel.toLowerCase()} {stats} {rotationHours} />
      <div class="scan-row">
