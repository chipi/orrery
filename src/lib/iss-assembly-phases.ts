/**
 * ISS assembly-phase data — dock events + truss / iROSA installation phases.
 *
 * Previously declared inline in `src/routes/iss/+page.svelte`. Moved here
 * so the route file stays focused on Svelte + Three.js plumbing and the
 * data becomes independently importable + testable.
 *
 * Each entry's `id` matches a `userData.animModuleId` tag assigned to the
 * corresponding visiting-craft / truss segGroup / wingPair Group / iROSA
 * Mesh in `src/lib/iss-proxy-model.ts`. The assembly walker animates each
 * part in at its real launch / install date instead of from frame 0.
 *
 * Companion data sources:
 *   - `getIssModules()` — pressurised modules (Zarya, Zvezda, …)
 *   - `getIssVisitors()` — flagship missions chosen as the synthetic
 *     "first-arrival" representatives for each visiting-craft family
 *
 * Note on "iROSA arrays": the 3 iROSA roll-out campaigns (Expeditions 65,
 * 67, 69) live inside `ISS_TRUSS_PHASES` as rows (ids prefixed
 * `truss-irosa-…`), not a separate array — they're physically mounted on
 * the existing truss mast bases and animate in via the same walker.
 */

/** Synthetic dock event for the 7 visiting-craft first-arrival missions. */
export interface IssDockEvent {
  id: string;
  name: string;
  launcher: string;
  launch_date: string;
...
/** Truss-segment install (STS) or iROSA EVA roll-out campaign. */
export interface IssTrussPhase {
  id: string;
  name: string;
  launcher: string;
  launch_date: string;
...
/**
 * Synthetic dock fly-ins. Each entry's first-of-kind flagship dock so
 * the chip narrative can name the crew / cargo. `id` prefix `dock-` is
 * mapped to the visitor id by the assembly walker.
 */
export const ISS_DOCK_EVENTS: readonly IssDockEvent[] = [
  {
    id: 'dock-soyuz_ms',
...
/**
 * Truss-segment STS install flights (10 segments) + the 3 iROSA EVA
 * roll-out campaigns (`truss-irosa-*`) that mount on the existing mast
 * bases. Animated in chronological order by the assembly walker.
 */
export const ISS_TRUSS_PHASES: readonly IssTrussPhase[] = [
  {
    id: 'truss-z1',
