...
export function createExploreSceneHost(bridge: any, deps: any) {
  ...
  async function resolveDeepSkyDeepLink(designation: string): Promise<void> {
    ...
    await ensureNeighborhood();
    const obj = bridge.deepSkyObjects.find(
      (o: any) => o.designation === designation || o.id === designation,
    );
  ...
  if (import.meta.env.DEV) {
    ...
    (window as unknown as { __exploreEnterSystem?: (id: string) => void }).__exploreEnterSystem = (
      id: string,
    ) => void enterBodyScene(id);
    (
      window as unknown as { __exploreSelectExoplanet?: (id: string) => void }
    ).__exploreSelectExoplanet = (id: string) => selectExoplanet(id);
    (window as unknown as { __exploreEnterDeepSky?: (id: string) => void }).__exploreEnterDeepSky =
      (id: string) => enterDeepSky(id);
    (window as unknown as { __exploreEnterMilkyWay?: () => Promise<void> }).__exploreEnterMilkyWay =
      async () => {
        if (!inNeighborhood()) await crossOutToNeighborhood();
  ...
  const onTourDrag = (e: Event): void => {
    const d = (e as CustomEvent).detail as { durationMs?: number; rotateRad?: number } | undefined;
  ...
  const loop = createAnimateLoop({
    onFrame: () => {
      ...
      if (!deps.getReducedMotion() && !bridge.simPaused)
        simT += (dt * bridge.simSpeed) / DAYS_PER_YEAR;
