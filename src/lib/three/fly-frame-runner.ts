...
export function createFlyFrameRunner(bridge: any, refs: any) {
  const {
    ...
    function draw2d() {
      ...
      const useCislunar2D =
        bridge.isMoonMission && bridge.cislunarTrajectory != null && bridge.cislunarTrajectory.phases.length > 0;
      ...
      drawSplit(bridge.retPts, Math.max(0, (sc.progress - 0.5) / 0.5), '#9966ff', 'rgba(153,102,255,0.2)');
      ...
      const heading = spacecraftHeading(bridge.simDay, bridge.arcTimeline, bridge.outPts, bridge.retPts);
        ...
        if (bridge.isMoonMission) {
          for (const mk of bridge.phaseMarkers) {
            next.push({
              ...
              reveal: markerStateFor(mk.event.met_days ?? 0, simMet, { reducedMotion: bridge.reducedMotion }),
    ...
    function frame() {
        ...
        if (bridge.showLaunch && bridge.isPlaying) {
          ...
          bridge.launchT = Math.min(bridge.launchDurationS, bridge.launchT + dt * bridge.launchSpeed * slow);
        }
        ...
        if (bridge.showDescent && bridge.isPlaying) {
          ...
          const rate = earthReentry ? (bridge.descentDurationS / 30) * (bridge.descentSpeed / 3) : bridge.descentSpeed;
        ...
        if (
          ...
          flyCam.camR +=
            (flyCam.helioAutoZoomTargetR + zoomOsc - flyCam.camR) * 0.005 * simSpeedFactor;
        ...
        if (!bridge.isMoonMission && !bridge.reducedMotion && !bridge.isDrag && bridge.openingActive) {
          const elapsedO = bridge.openingStartedAt > 0 ? performance.now() - bridge.openingStartedAt : 0;
        ...
        if (sensory.active('audio')) {
          ...
          const p1 = spacecraftPos(bridge.simDay + 0.5, bridge.arcTimeline, bridge.outPts, bridge.retPts).pos;
        ...
        if (outLine) outLine.visible = (!afterArrival || bridge.epilogueActive) && !inFlybyCinemaForLines;
        if (retLine)
          retLine.visible =
            (!afterArrival || bridge.epilogueActive) && bridge.retPts.length >= 2 && !inFlybyCinemaForLines;
        ...
        if (coastLine.visible && bridge.outPts.length >= 2) {
          ...
          const t =
            total > 0 ? Math.max(0, Math.min(1, (bridge.simDay - bridge.arcTimeline.dep_day) / total)) : 0;
        ...
        if (velocityArrow.visible || centripetalArrow.visible) {
          const sc1 = spacecraftPos(bridge.simDay + 0.5, bridge.arcTimeline, bridge.outPts, bridge.retPts).pos;
        ...
        if (!bridge.isMoonMission && bridge.mission.flight?.events) {
          ...
          const activeBurn = findActiveBurn(bridge.mission.flight.events, bridge.simDay - bridge.arcTimeline.dep_day);
          if (activeBurn) {
        ...
        if (bridge.view === '3d' && bridge.container) {
          ...
          const anyCislunarLayerOn =
            ...
            conicsLayerOn;
          if (anyCislunarLayerOn && bridge.cislunarTrajectory && bridge.cislunarTrajectory.phases.length > 0) {
