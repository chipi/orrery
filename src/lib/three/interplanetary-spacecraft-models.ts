...
function buildCassini(): THREE.Group {
  ...
  const bus = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.5, 8), heroGold());
  bus.rotation.z = Math.PI / 2;
  g.add(bus);
  // HGA dish — 0.5u radius cone facing +X. White contrasts the gold
  // bus + boom so the silhouette reads as multi-toned. DoubleSide so
  // the concave face is solid from all viewing angles.
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.48, 0.2, 16, 1, true), dishMat);
  dish.rotation.z = -Math.PI / 2;
  dish.position.x = 0.4;
  g.add(dish);
  // Huygens probe — small disc strapped to the side of the bus.
  // Lighter painted-bus skin so it reads distinct from the gold bus.
  ...
  for (let i = 0; i < 3; i++) {
    const rtg = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 0.18, 8), heroDark());
    rtg.position.set(-0.4, -0.05 + i * 0.05, -0.2 + i * 0.2);
    rtg.rotation.x = Math.PI / 2;
    g.add(rtg);
  }
  // Magnetometer boom — thin long cylinder out the -Y axis.
  const mag = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 1.6, 6), heroMetal(0x9aa0a8));
...
function buildVoyager(): THREE.Group {
  const g = new THREE.Group();
  // HGA dish — the dominant feature, 0.6u radius. DoubleSide so the
  // concave interior face renders solid from behind.
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.6, 0.15, 24, 1, true), dishMat);
  dish.rotation.z = -Math.PI / 2;
  g.add(dish);
  // Bus — small box behind the dish.
  const bus = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.2, 0.2), heroWhite(0xbfc3c9));
  bus.position.x = -0.2;
  g.add(bus);
  // RTG boom — 3 RTGs on a short tube extending sideways.
  for (let i = 0; i < 3; i++) {
    const rtg = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 0.14, 8), heroDark());
    rtg.position.set(-0.2 - i * 0.18, 0, -0.5);
    rtg.rotation.x = Math.PI / 2;
    g.add(rtg);
  }
  // Magnetometer boom — long thin tube the other way.
  const mag = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.015, 1.8, 6), heroMetal(0x9aa0a8));
  mag.position.set(-0.2, 0.9, 0);
  g.add(mag);
  // Golden record — a small gold disc on the side facing +Y.
  const record = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 0.02, 16), heroGold());
...
function buildGalileo(): THREE.Group {
  ...
  const spin = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.3, 12), heroWhite(0xbfc3c9));
  spin.position.y = 0.2;
  g.add(spin);
  // Despun section (bottom) — slightly wider cylinder, gold MLI wrapped.
  const despun = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.22, 0.25, 12), heroGold());
  despun.position.y = -0.1;
  g.add(despun);
  // HGA — partially-deployed umbrella ribs (only 12 of 18 ribs out).
  const ribsGroup = new THREE.Group();
  for (let i = 0; i < 12; i++) {
    const rib = new THREE.Mesh(new THREE.CylinderGeometry(0.01, 0.01, 0.5, 4), heroMetal(0x9aa0a8));
  ...
  const mag = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.015, 1.2, 6), heroMetal(0x9aa0a8));
  mag.position.y = 1.0;
  g.add(mag);
  ...
  const probeBoom = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xb45c5c, metalness: 0.3, roughness: 0.5 }),
  );
...
function buildNewHorizons(): THREE.Group {
  ...
  const bus = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.3, 0.3), heroWhite(0xbfc3c9));
  g.add(bus);
  // HGA dish — large, 0.5u radius. DoubleSide so concave face is solid.
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.45, 0.12, 16, 1, true), dishMat);
  dish.rotation.z = -Math.PI / 2;
  dish.position.x = 0.3;
  g.add(dish);
  // RTG — single big cylinder on the opposite side.
  const rtg = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 0.5, 12), heroDark());
...
function buildJuno(): THREE.Group {
  const g = new THREE.Group();
  // Hexagonal bus — short flat cylinder with 6 sides. Juno's bus is
  // gold MLI wrapped — the gold hub reads correctly against the blue wings.
  const bus = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.18, 6), heroGold());
  g.add(bus);
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let s = 0; s < 4; s++) {
      const panel = new THREE.Mesh(new THREE.BoxGeometry(0.32, 0.02, 0.22), heroSolar(4));
  ...
  // HGA dish on top, axis along +Y. DoubleSide for concave face.
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.18, 0.1, 16, 1, true), dishMat);
  dish.position.y = 0.18;
  g.add(dish);
  // Magnetometer boom — thin tube sideways from one arm root.
  const mag = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.6, 6), heroMetal(0x9aa0a8));
...
function buildBepiColombo(): THREE.Group {
  const g = new THREE.Group();
  // MTM body — long cylindrical bus at the back end, gold MLI wrapped.
  const mtm = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.16, 0.5, 10), heroGold());
  mtm.rotation.z = Math.PI / 2;
  mtm.position.x = -0.35;
  g.add(mtm);
  // MTM solar arrays — two long wings extending in Z.
  for (const side of [-1, 1]) {
    const wing = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.02, 0.18), heroSolar(4));
    wing.position.set(-0.35, 0, side * 0.45);
    g.add(wing);
  }
  // MPO — cube bus in the middle (painted white panels, not gold foil).
  const mpo = new THREE.Mesh(new THREE.BoxGeometry(0.26, 0.26, 0.26), heroWhite(0xbfc3c9));
  mpo.position.x = 0.05;
  g.add(mpo);
  // MPO HGA dish on +X side. DoubleSide for concave face.
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.18, 0.08, 14, 1, true), dishMat);
  dish.rotation.z = -Math.PI / 2;
  dish.position.set(0.25, 0.05, 0);
  g.add(dish);
  // Mio (Japanese magnetospheric orbiter) — squat cylinder on top, gold MLI.
  const mio = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 0.18, 14), heroGold());
...
function buildDawn(): THREE.Group {
  const g = new THREE.Group();
  // Bus — small cube.
  const bus = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.25, 0.22), heroWhite(0xbfc3c9));
  g.add(bus);
  // Solar arrays — two long wings.
  for (const side of [-1, 1]) {
    const wing = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.16, 1.1), heroSolar(4));
    wing.position.z = side * 0.7;
    g.add(wing);
  }
  // HGA on top. DoubleSide for concave face.
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.16, 0.07, 14, 1, true), dishMat);
  dish.position.y = 0.2;
  g.add(dish);
...
function buildGiotto(): THREE.Group {
  ...
  g.add(shield);
  // Drum bus — short cylinder, gold MLI wrapped (Giotto's real livery).
  const bus = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.22, 0.34, 14), heroGold());
  bus.rotation.z = Math.PI / 2;
  g.add(bus);
  // Solar drum belt — open cylinder around bus circumference. DoubleSide
  // so the thin open shell renders from inside and outside.
  const beltMat = heroSolar();
  beltMat.side = THREE.DoubleSide;
  const belt = new THREE.Mesh(new THREE.CylinderGeometry(0.23, 0.23, 0.18, 14, 1, true), beltMat);
  belt.rotation.z = Math.PI / 2;
  g.add(belt);
  // HGA dish out the back. DoubleSide for concave face.
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.22, 0.1, 14, 1, true), dishMat);
  dish.rotation.z = Math.PI / 2;
...
function buildHayabusa2(): THREE.Group {
  const g = new THREE.Group();
  // Bus — small box, gold MLI wrapped.
  const bus = new THREE.Mesh(new THREE.BoxGeometry(0.32, 0.32, 0.22), heroGold());
  g.add(bus);
  // Solar panels — two short wings on Z axis.
  for (const side of [-1, 1]) {
    const panel = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.28, 0.5), heroSolar());
    panel.position.z = side * 0.4;
    g.add(panel);
  }
  // HGA dish on +Y. DoubleSide for concave face.
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.14, 0.07, 14, 1, true), dishMat);
  ...
  const eng = new THREE.Mesh(new THREE.ConeGeometry(0.06, 0.1, 8), heroDark());
...
function buildJuice(): THREE.Group {
  const g = new THREE.Group();
  // Bus — compact box.
  const bus = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.36, 0.3), heroWhite(0xbfc3c9));
  g.add(bus);
  // Solar arrays — two MASSIVE wings (JUICE's signature feature).
  for (const side of [-1, 1]) {
    const wing = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.32, 1.4), heroSolar(4));
  ...
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.22, 0.1, 16, 1, true), dishMat);
  ...
  const mag = new THREE.Mesh(new THREE.CylinderGeometry(0.01, 0.01, 0.55, 6), heroMetal(0x9aa0a8));
...
function buildPioneer(): THREE.Group {
  const g = new THREE.Group();
  // HGA dish — large 0.6u radius, defining feature. DoubleSide for
  // concave face.
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.6, 0.18, 24, 1, true), dishMat);
  dish.rotation.z = -Math.PI / 2;
  g.add(dish);
  // Compact bus behind the dish — hexagonal shape (Pioneer hexagon).
  const bus = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.16, 0.16, 6), heroWhite(0xbfc3c9));
  bus.rotation.z = Math.PI / 2;
  bus.position.x = -0.18;
  g.add(bus);
  // Two RTGs on a short boom extending -X.
  for (let i = 0; i < 2; i++) {
    const rtg = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 0.18, 8), heroDark());
    rtg.position.set(-0.4 - i * 0.18, 0, 0.18);
    rtg.rotation.x = Math.PI / 2;
    g.add(rtg);
  }
  // Magnetometer boom — long thin tube sideways.
  const mag = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 1.4, 6), heroMetal(0x9aa0a8));
  mag.position.set(-0.18, 0.7, 0);
  g.add(mag);
  // Gold plaque on the bus — Pioneer's iconic Earth message.
  const plaque = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.1, 0.07), heroGold());
...
function buildRosetta(): THREE.Group {
  const g = new THREE.Group();
  // Bus — cube, gold MLI wrapped (Rosetta's real livery).
  const bus = new THREE.Mesh(new THREE.BoxGeometry(0.32, 0.32, 0.32), heroGold());
  g.add(bus);
  // Solar arrays — two LONG wings (Rosetta's signature).
  for (const side of [-1, 1]) {
    const wing = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.22, 1.3), heroSolar(4));
    wing.position.z = side * 0.85;
    g.add(wing);
  }
  // HGA dish on +X. DoubleSide for concave face.
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.24, 0.1, 16, 1, true), dishMat);
  dish.rotation.z = -Math.PI / 2;
  dish.position.x = 0.28;
  g.add(dish);
  // Philae lander — small box on -Y side of bus.
  const philae = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 0.12, 6), heroWhite(0xbfc3c9));
...
function buildUlysses(): THREE.Group {
  const g = new THREE.Group();
  // Bus — squat box.
  const bus = new THREE.Mesh(new THREE.BoxGeometry(0.32, 0.26, 0.3), heroWhite(0xbfc3c9));
  g.add(bus);
  // HGA dish on +Y (spin axis). DoubleSide for concave face.
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.34, 0.14, 18, 1, true), dishMat);
  dish.position.y = 0.26;
  g.add(dish);
  // RTG cylinder — one big GPHS-RTG on +X side.
  const rtg = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.09, 0.4, 10), heroDark());
...
function buildVega(): THREE.Group {
  ...
  const bus = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.5, 12), heroWhite(0xbfc3c9));
  g.add(bus);
  // Solar wings — two short panels off ±Z.
  for (const side of [-1, 1]) {
    const wing = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.22, 0.5), heroSolar());
    wing.position.z = side * 0.4;
    g.add(wing);
  }
  // Descent capsule on top — sphere (Venera-style lander), gold MLI wrapped.
  const capsule = new THREE.Mesh(new THREE.SphereGeometry(0.18, 18, 10), heroGold());
  ...
  g.add(balloon);
  // HGA dish at the bottom (transit-comms with Earth). DoubleSide for
  // concave face.
  const dishMat = heroWhite(0xeef0f2);
  dishMat.side = THREE.DoubleSide;
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.22, 0.08, 14, 1, true), dishMat);
...
function buildVenera13(): THREE.Group {
  const g = new THREE.Group();
  // Bus — tall cylinder.
  const bus = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.16, 0.42, 12), heroWhite(0xbfc3c9));
  g.add(bus);
  // Two solar panels — flat wings on ±X.
  for (const side of [-1, 1]) {
    const panel = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.18, 0.03), heroSolar());
    panel.position.x = side * 0.36;
    g.add(panel);
  }
  // Lander sphere — iconic Venera ball, dominant feature. Gold MLI.
  const lander = new THREE.Mesh(new THREE.SphereGeometry(0.22, 18, 12), heroGold());
  lander.position.y = 0.35;
  g.add(lander);
  // Antenna boom — long thin tube extending out the bottom.
  const ant = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.015, 0.7, 6), heroMetal(0x9aa0a8));
  ant.position.y = -0.45;
  g.add(ant);
  return g;
}
