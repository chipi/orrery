<script lang="ts">
  ...
  /*
   * EarthOrbitalScene — the bespoke orbital-view of /earth (LEO/MEO/GEO
   * regime rings, satellite + ISS + Moon-orbiter markers, atmosphere
   * shell, ozone-hole layer, science chips). Extracted from the route
   * file in #285 Phase 2 B2 so /earth can switch between this orbital
   * mode and the new SurfaceScene 'earth' surface-mode (launchpads).
   *
   * Mounted by /earth/+page.svelte under `{#if mode === 'orbital'}`.
   * Toggling mode unmounts this component and tears down its Three.js
   * scene via onDestroy. Remounting on the next mode toggle resets
   * the orbital view to its default camera, same as the RESET VIEW
   * button does within a single session.
   *
   * `<svelte:head>` lives in the route file so the page title stays
   * constant across both modes. ScienceLayersPanel (atmosphere shell +
   * ozone-hole layers) stays inside this component because both
   * overlays are orbital-mode-only features.
   */
</script>
