<script lang="ts">
  ...
  // small floating modal where the user picks a graphics quality tier
  // (auto / minimal / low / medium / high / cinematic). Choice is
  // written to localStorage via writeUserChoice; the renderer reads it
  // synchronously in onMount, so taking effect requires a reload —
  // shown as a hint after the change.
...
<div class="fly" class:hud-hidden={hudHidden}>
  ...
  <!-- Settings — wave 2/3 punch #3. Gear icon top-right; click opens
       a small floating panel with quality-tier radios. Choice persists
       to localStorage and applies on reload. -->
  <button
    type="button"
    class="settings-btn"
