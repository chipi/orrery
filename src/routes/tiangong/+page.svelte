<script lang="ts">
  ...
  function startThree() {
    ...
    // navigating in/out of /tiangong piled up GBs of GPU memory and
    // throttled rendering. Mirrors disposeSceneRenderer on the surface routes.
...
<div class="tiangong-root">
  ...
  {:else}
    ...
    <div class="hud-top-mobile" role="group" aria-label={m.tiangong_hud_aria()}>
      <button
        type="button"
        class="toggle"
        data-testid="tiangong-blueprint-toggle-mobile"
      ...
      {#if viewMode !== 'list'}
        <button
          type="button"
          class="toggle"
          data-testid="tiangong-reset-camera-mobile"
          onclick={() => resetCamera()}
