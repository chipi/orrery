...
async function startTour(page: Page) {
  ...
  // Three tour launchers now (curator / extended / kiosk). Target the curator
  // one by its positive class so adding a launcher never re-breaks this.
  const tourBtn = page.locator(`${OVERLAY} .tour-start-curator`);
...
test.describe('pale-blue-dot guided stages (PRD-016 §S11 pilot)', () => {
  ...
  test('data-audio-stage anchors are present on / for every non-cue stage', async ({ page }) => {
    ...
    for (const name of ['hero', 'hero-illustration', 'route-grid', 'route-card-solar-system']) {
  ...
  test('route-grid flash fires at 100s and route-card-solar-system flash at 110s', async ({
    page,
  }) => {
    await startTour(page);
    ...
    await expect(page.locator('[data-audio-stage="route-card-solar-system"]')).toHaveClass(
      /audio-stage-flash/,
      { timeout: 2000 },
    );
  });
