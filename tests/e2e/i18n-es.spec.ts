 ...
 *
 * Loads each of the six screens at `/es/<path>`, asserts the locale
 ...
 *   - The locale picker fails to apply the URL locale on hydration.
 */
...
test.describe('/es/ URL smoke', () => {
  ...
  test('en-US fallback still works on bare URL (baseLocale)', async ({ page }) => {
  ...
  test('selected locale persists when navigating via top nav', async ({ page }) => {
    await page.goto('/es/explore', { waitUntil: 'networkidle' });
    await expect(localeChip(page).first()).toContainText('ES', { timeout: 10_000 });
